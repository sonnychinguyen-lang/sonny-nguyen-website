...
export function disablePickingIndex(disabledPickingIndices, objectIndex) {
    if (disabledPickingIndices.length === PICKING_MAX_DISABLED_INDICES) {
        log.warn(`pickMultipleObjects can only exclude ${PICKING_MAX_DISABLED_INDICES} previously picked objects for layers without picking buffers`)();
    }
    else {
...
`;
function addPickingUniformsGLSL(source) {
    return source.replace('  vec4 highlightColor;\n} picking;', `  vec4 highlightColor;\n${pickingUniformsGLSL}} picking;`);
}
...
export default {
    ...
    getUniforms(props, prevUniforms) {
        const uniforms = picking.getUniforms(props, prevUniforms);
