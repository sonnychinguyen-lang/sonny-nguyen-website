...
export function isGlobeNorthUp(bearing) {
...
class GlobeViewport extends Viewport {
    constructor(opts = {}) {
        const { longitude = 0, bearing = 0, pitch = 0, zoom = 0, 
    ...
    _getRayToGlobe(screenPosition, { topLeft = true, targetZ } = {}) {
        ...
        const triangleAreaSquared = (4 * rayStartDistanceSquared * rayEndDistanceSquared -
    ...
    getZoomAnchorStrength(screenPosition) {
        ...
        const edgeProgress = Math.max(0, Math.min(1, (distanceRatio - GLOBE_ZOOM_ANCHOR_DAMPING_START_RATIO) /
    ...
    panByPosition(coordinates, screenPosition, dragStartPosition) {
        if (!dragStartPosition) {
            ...
            const crossesPole = Math.abs(currentCoordinates[1]) > MAX_LATITUDE || Math.abs(longitudeDelta) > 90;
...
export function zoomAdjust(latitude, clampToPoles) {
    if (clampToPoles) {
