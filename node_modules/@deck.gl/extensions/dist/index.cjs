...
var PATH_STYLE_ATTRIBUTES = ["instanceDashArrays", "instanceDashOffsets", "instanceOffsets"];
function dashMetricsProjectionPropsChanged({ props, oldProps }) {
  return !(0, import_core6._deepEqual)(props.modelMatrix, oldProps.modelMatrix, 2) || props.coordinateSystem !== oldProps.coordinateSystem || !(0, import_core6._deepEqual)(props.coordinateOrigin, oldProps.coordinateOrigin, 1);
}
function projectRenderedPathPosition(layer, position, projectUniforms) {
  const viewport = layer.context.viewport;
  if (!(viewport instanceof import_core6.WebMercatorViewport) || viewport.zoom < 12) {
    return layer.projectPosition(position, { autoOffset: false });
  }
  const positionWorld = import_core7.vec4.transformMat4([], [position[0], position[1], position[2] || 0, 1], projectUniforms.modelMatrix);
  const coordinateSystem = layer.props.coordinateSystem === "default" ? "lnglat" : layer.props.coordinateSystem;
  if (coordinateSystem === "lnglat") {
    ...
    import_core7.vec3.sub(positionWorld, positionWorld, projectUniforms.coordinateOrigin);
  } else if (coordinateSystem === "cartesian") {
    import_core7.vec3.sub(positionWorld, positionWorld, projectUniforms.coordinateOrigin);
  }
  const deltaLatitude = positionWorld[1];
  const scale = import_core7.vec3.scaleAndAdd([], projectUniforms.commonUnitsPerWorldUnit, projectUniforms.commonUnitsPerWorldUnit2, deltaLatitude);
  return import_core7.vec3.multiply([], positionWorld, scale);
}
var PathStyleExtension = class extends import_core6.LayerExtension {
  constructor({ dash = false, offset = false, dashMode, highPrecisionDash = false } = {}) {
    const resolvedDashMode = dashMode ?? (highPrecisionDash ? "path" : "segment");
    super({
      dash: dash || highPrecisionDash || dashMode !== void 0,
      offset,
  ...
  updateState(params, extension) {
    ...
    if (extension.opts.dash) {
      const layerType = extension.getLayerType(this);
      if (layerType === "path" && extension.opts.dashMode === "path" && dashMetricsProjectionPropsChanged(params)) {
        (_a = this.getAttributeManager()) == null ? void 0 : _a.invalidate("instanceDashOffsets");
      }
      if (layerType === "scatterplot" || layerType === "textBackground") {
  ...
  calculateDashMetrics(attribute, { startRow, endRow }) {
    var _a, _b, _c;
    const pathTesselator = this.state.pathTesselator;
    const vertexStarts = pathTesselator == null ? void 0 : pathTesselator.vertexStarts;
    const instanceCount = pathTesselator == null ? void 0 : pathTesselator.instanceCount;
    const packedPositions = pathTesselator == null ? void 0 : pathTesselator.get("positions");
    if (!vertexStarts || instanceCount === void 0 || typeof (pathTesselator == null ? void 0 : pathTesselator.getPathSegmentIndices) !== "function") {
      throw new Error("PathStyleExtension requires PathLayer tessellation data for path dashes.");
    }
    const binaryPath = (_b = (_a = this.props.data) == null ? void 0 : _a.attributes) == null ? void 0 : _b.getPath;
    const binaryValue = ArrayBuffer.isView(binaryPath) ? binaryPath : binaryPath == null ? void 0 : binaryPath.value;
    const hasReadableBinaryPath = ArrayBuffer.isView(binaryValue);
    const usePackedPositions = packedPositions && (!binaryPath || pathTesselator.normalize || ((_c = pathTesselator.opts) == null ? void 0 : _c.isWebGPU));
    if (binaryPath && !hasReadableBinaryPath || !usePackedPositions && !hasReadableBinaryPath) {
      throw new Error("PathStyleExtension cannot calculate whole-path dash metrics from GPU-only getPath data; supply data.attributes.instanceDashOffsets.");
    }
    ...
    if (!usePackedPositions) {
      const bytesPerElement = binaryValue.BYTES_PER_ELEMENT;
      binarySize = (binaryPath == null ? void 0 : binaryPath.size) || (this.props.positionFormat === "XY" ? 2 : 3);
      binaryStride = (binaryPath == null ? void 0 : binaryPath.stride) ? binaryPath.stride / bytesPerElement : binarySize;
      binaryOffset = (binaryPath == null ? void 0 : binaryPath.offset) ? binaryPath.offset / bytesPerElement : 0;
    }
    const sourcePosition = new Array(3);
    const projectUniforms = import_core6.project.getUniforms({
    ...
    const readPosition = (vertexIndex) => {
      if (usePackedPositions) {
    ...
    for (let row = firstRow; row < lastRow; row++) {
      ...
      for (const segmentIndex of validSegments) {
        ...
        const start = segmentIndex === previousSegment + 1 && previousEnd ? previousEnd : projectRenderedPathPosition(this, readPosition(segmentIndex), projectUniforms);
        const end = projectRenderedPathPosition(this, readPosition(segmentIndex + 1), projectUniforms);
        phase += import_core7.vec3.dist(start, end);
...
var FILL_UV_SCALE = 512 / 4e7;
function getPatternUnitScale(sizeUnits, viewport) {
  switch (sizeUnits) {
    case "pixels":
...
function getPatternUniforms(opts) {
  ...
  if ("project" in opts) {
    const { fillPatternMask = true, fillPatternEnabled = true, fillPatternSizeUnits = "meters", fillPatternCommonFrame = null, procedural = false } = opts;
    ...
    const origin = [coordinateOriginCommon[0], coordinateOriginCommon[1]];
    ...
    uniforms.uvCoordinateOrigin64Low = [(0, import_core8.fp64LowPart)(origin[0]), (0, import_core8.fp64LowPart)(origin[1])];
    ...
    uniforms.flipY = ((_a = opts.project.viewport) == null ? void 0 : _a.flipY) ?? false;
  }
  return uniforms;
}
var patternShaders = {
...
// dist/fill-style/procedural-pattern.js
var PROCEDURAL_PATTERN_TEXTURE_FORMAT = "rg32float";
var PROCEDURAL_PATTERN_COMPONENTS_PER_TEXEL = 2;
var PROCEDURAL_PATTERN_TEXELS_PER_PATTERN = 4;
var PROCEDURAL_PATTERN_TYPES = {
  none: 0,
  hatch: 1,
  crossHatch: 2,
  dots: 3
};
var PROCEDURAL_PATTERN_OFFSETS = {
  type: 0,
  size: 1,
  gap0: 2,
  gap1: 3,
...
var COMPONENTS_PER_PATTERN = PROCEDURAL_PATTERN_COMPONENTS_PER_TEXEL * PROCEDURAL_PATTERN_TEXELS_PER_PATTERN;
var DEGREES_TO_RADIANS = Math.PI / 180;
var DEFAULT_SIZE = 1;
var DEFAULT_GAP = 1;
var DEFAULT_HATCH_ANGLE = 0;
var DEFAULT_CROSS_HATCH_ANGLES = [45, 135];
var DEFAULT_DOT_ANGLE = 0;
var DEFAULT_DOT_SKEW = 0;
function packProceduralPatterns(mapping) {
  const entries = Object.entries(mapping);
  const data = new Float32Array((entries.length + 1) * COMPONENTS_PER_PATTERN);
  const patternIndices = /* @__PURE__ */ new Map();
  entries.forEach(([id, config], entryIndex) => {
    ...
    switch (config.type) {
      case "hatch": {
        const gap = config.gap === void 0 ? DEFAULT_GAP : config.gap;
        const strokeWidth = config.strokeWidth === void 0 ? DEFAULT_SIZE : config.strokeWidth;
        const angle = config.angle === void 0 ? DEFAULT_HATCH_ANGLE : config.angle;
      ...
      case "cross-hatch": {
        const angles = config.angles === void 0 ? DEFAULT_CROSS_HATCH_ANGLES : config.angles;
        const strokeWidth = config.strokeWidth === void 0 ? DEFAULT_SIZE : config.strokeWidth;
        const gap = config.gap === void 0 ? DEFAULT_GAP : config.gap;
      ...
      case "dots": {
        const radius = config.radius === void 0 ? DEFAULT_SIZE : config.radius;
        const gap = config.gap === void 0 ? DEFAULT_GAP : config.gap;
        const angle = config.angle === void 0 ? DEFAULT_DOT_ANGLE : config.angle;
        const skew = config.skew === void 0 ? DEFAULT_DOT_SKEW : config.skew;
...
function validateSkew(value, name) {
...
var FillStyleExtension = class extends import_core9.LayerExtension {
  ...
  initializeState(context, extension) {
    ...
    if (extension.opts.pattern) {
      attributeManager.add({
        fillPatternFrames: {
          ...
          transform: extension.opts.proceduralPattern ? extension.getProceduralPatternIndex.bind(this) : extension.getPatternFrame.bind(this)
        },
        fillPatternScales: {
          size: 1,
          stepMode: "dynamic",
    ...
    this.setState({
      ...
      ...extension.opts.proceduralPattern && extension.createProceduralPatternTexture.call(this, {})
    });
  }
  updateState({ props, oldProps }, extension) {
    ...
    if (props.fillPatternMapping && props.fillPatternMapping !== oldProps.fillPatternMapping) {
      if (extension.opts.proceduralPattern && typeof props.fillPatternMapping !== "string") {
        ...
        oldTexture == null ? void 0 : oldTexture.delete();
      }
      this.getAttributeManager().invalidate("getFillPattern");
    }
    if (props.fillPatternMapping !== oldProps.fillPatternMapping || props.getFillPatternScale !== oldProps.getFillPatternScale) {
  ...
  draw(params, extension) {
    ...
    const fillProps = {
      ...
      fillPatternTexture: extension.opts.proceduralPattern ? this.state.proceduralPatternTexture || this.state.emptyTexture : fillPatternAtlas || this.state.emptyTexture,
  ...
  finalizeState() {
    ...
    proceduralPatternTexture == null ? void 0 : proceduralPatternTexture.delete();
  }
  ...
  getProceduralPatternIndex(name) {
    ...
    return [(patternIndices == null ? void 0 : patternIndices.get(name)) ?? 0, 0, 0, 0];
  }
...
var MERCATOR_REFERENCE_VIEWPORT = new import_core18.WebMercatorViewport({
  width: 1,
  height: 1,
  longitude: 0,
  latitude: 0,
...
function lngLatToMercatorCommon(lngLat) {
  const [x, y] = MERCATOR_REFERENCE_VIEWPORT.projectPosition(lngLat);
  return [x, y];
}
function getMercatorReferenceViewport(viewport) {
...
function makeViewport(opts) {
  ...
  const centerWorld = isGeospatial ? MERCATOR_REFERENCE_VIEWPORT.unprojectPosition([
    (bounds[0] + bounds[2]) / 2,
    (bounds[1] + bounds[3]) / 2,
    0
...
var MaskEffect = class {
  ...
  preRender({ layers, layerFilter, viewports, onViewportActive, views, effects, isPicking }) {
    let didRender = false;
    if (isPicking) {
      return { didRender };
    }
  ...
  /* eslint-disable-next-line complexity */
  _renderChannel(channelInfo, { layerFilter, onViewportActive, views, effects, viewport, viewportChanged }) {
    let didRender = false;
...
var import_core28 = require("@deck.gl/core");

// dist/terrain/terrain-effect.js
var import_core27 = require("@deck.gl/core");
...
`,
  fs: `${uniformBlock5}in vec2 terrainMercPos;
  ...
  getUniforms: (opts = {}) => {
    ...
    if ("terrainSkipRender" in opts || "drawToTerrainHeightMap" in opts) {
      ...
      if (drawToTerrainHeightMap) {
        ...
        const coverTexture = fbo == null ? void 0 : fbo.colorAttachments[0].texture;
        if (opts.isPicking) {
...
var TerrainCover = class {
  ...
  _updateViewport(viewport) {
    ...
    if (this.tile && "boundingBox" in this.tile) {
      ...
      this.targetBoundsCommon = joinLayerBounds([targetLayer], getMercatorReferenceViewport(viewport));
    }
    ...
    if (this.tile) {
      ...
      const newBounds = viewport instanceof import_core23._GlobeViewport ? this.targetBoundsCommon : getRenderBounds(this.targetBoundsCommon, viewport);
...
var HeightMapBuilder = class {
  ...
  shouldUpdate({ layers, viewport }) {
    ...
    if (!this.layersBoundsCommon) {
      ...
      const bounds = viewport instanceof import_core26._GlobeViewport ? this.layersBoundsCommon : getRenderBounds(this.layersBoundsCommon, viewport);
      if (bounds[2] <= bounds[0] || bounds[3] <= bounds[1]) {
        this.renderViewport = null;
        return false;
      }
      ...
      const centerMerc = viewport.isGeospatial ? lngLatToMercatorCommon([
...
var TerrainEffect = class {
  ...
  setup({ device, deck }) {
    ...
    if (HeightMapBuilder.isSupported(device)) {
      ...
      import_core27.log.warn("Terrain offset mode is not supported by this browser")();
    }
  ...
  preRender(opts) {
    ...
    const nonTerrainEffects = (_a = opts.effects) == null ? void 0 : _a.filter((e) => e !== this);
...
var TerrainExtension = class extends import_core28.LayerExtension {
  getShaders() {
    return {
