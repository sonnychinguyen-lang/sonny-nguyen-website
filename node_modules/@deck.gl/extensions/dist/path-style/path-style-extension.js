...
import { LayerExtension, WebMercatorViewport, project, _deepEqual as deepEqual, _mergeShaders as mergeShaders } from '@deck.gl/core';
...
function dashMetricsProjectionPropsChanged({ props, oldProps }) {
    ...
    return (!deepEqual(props.modelMatrix, oldProps.modelMatrix, 2) ||
...
function projectRenderedPathPosition(layer, position, projectUniforms) {
    ...
    const positionWorld = vec4.transformMat4([], [position[0], position[1], position[2] || 0, 1], projectUniforms.modelMatrix);
    ...
    const scale = vec3.scaleAndAdd([], projectUniforms.commonUnitsPerWorldUnit, projectUniforms.commonUnitsPerWorldUnit2, deltaLatitude);
    return vec3.multiply([], positionWorld, scale);
}
...
class PathStyleExtension extends LayerExtension {
    ...
    calculateDashMetrics(attribute, { startRow, endRow }) {
        ...
        const vertexStarts = pathTesselator?.vertexStarts;
        const instanceCount = pathTesselator?.instanceCount;
        ...
        const binaryPath = this.props.data?.attributes?.getPath;
        ...
        const usePackedPositions = packedPositions && (!binaryPath || pathTesselator.normalize || pathTesselator.opts?.isWebGPU);
        if ((binaryPath && !hasReadableBinaryPath) || (!usePackedPositions && !hasReadableBinaryPath)) {
            throw new Error('PathStyleExtension cannot calculate whole-path dash metrics from GPU-only getPath data; ' +
        ...
        for (let row = firstRow; row < lastRow; row++) {
            ...
            for (const segmentIndex of validSegments) {
                ...
                const start = segmentIndex === previousSegment + 1 && previousEnd
                    ? previousEnd
