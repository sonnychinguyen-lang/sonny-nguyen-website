...
var WMSLayer = class extends import_core3.CompositeLayer {
  ...
  _createImageSource(props) {
    ...
    if (typeof props.data === "string") {
      return (0, import_core4.createDataSource)(props.data, [import_wms.WMSSource], {
        core: {
...
function getOSMTileIndices(viewport, maxZ, zRange, bounds) {
  const project = viewport instanceof import_core5._GlobeViewport ? (
  ...
  const minZ = viewport instanceof import_core5.WebMercatorViewport && viewport.pitch <= 60 ? maxZ : 0;
  if (bounds) {
  ...
  if (viewport instanceof import_core5.WebMercatorViewport && viewport.subViewports && viewport.subViewports.length > 1) {
...
var Tileset2D = class {
  ...
  constructor(opts) {
    ...
    this._frameNumber = 0;
    this._modelMatrix = new import_core6.Matrix4();
    this._modelMatrixInverse = new import_core6.Matrix4();
  }
  /* Public API */
  get tiles() {
    return this._tiles;
  ...
  update(viewport, { zRange, modelMatrix } = {
    ...
    const modelMatrixAsMatrix4 = modelMatrix ? new import_core6.Matrix4(modelMatrix) : new import_core6.Matrix4();
    const isModelMatrixNew = !modelMatrixAsMatrix4.equals(this._modelMatrix);
    if (!this._viewport || !viewport.equals(this._viewport) || !(0, import_core6.equals)(this._zRange, zRange) || isModelMatrixNew) {
      if (isModelMatrixNew) {
    ...
    if (cullRect && this._viewport) {
      ...
      for (const [minX, minY, maxX, maxY] of boundsArr) {
        ...
        if ("west" in bbox) {
          ...
          if (modelMatrix && !import_core6.Matrix4.IDENTITY.equals(modelMatrix)) {
...
var TileLayer = class extends import_core7.CompositeLayer {
  ...
  renderLayers() {
    ...
    return this.state.tileset.tiles.map((tile) => {
      ...
      if (!tile.isLoaded && !tile.content) {
        ...
        tile.layers = (0, import_core7._flatten)(layers, Boolean).map((layer) => layer.clone({
          tile,
          ...subLayerProps
        }));
  ...
  filterSubLayer({ layer, cullRect }) {
    ...
    return this.state.tileset.isTileVisible(tile, cullRect, modelMatrix ? new import_core8.Matrix4(modelMatrix) : null);
  }
};
...
var uniformBlockGLSL = `layout(std140) uniform tripsUniforms {
  bool fadeTrail;
  float trailLength;
  float currentTime;
} trips;
...
function packTripTimestamps(timestamps, instanceCount = timestamps.length, isLoop = false) {
  ...
  for (let index = 0; index < instanceCount; index++) {
    ...
    const nextTimestampIndex = isClosed ? (timestampIndex + 1) % cycleLength : Math.min(timestampIndex + 1, timestamps.length - 1);
    packedTimestamps[index * 2] = timestamps[timestampIndex];
    packedTimestamps[index * 2 + 1] = timestamps[nextTimestampIndex];
  }
  return packedTimestamps;
}
var tripsShaderInjectionsWGSL = {
...
var TripsLayer = class extends import_layers5.PathLayer {
  getShaders() {
    ...
    shaders.inject = this.context.device.type === "webgpu" ? tripsShaderInjectionsWGSL : tripsShaderInjectionsGLSL;
...
var H3HexagonLayer = class _H3HexagonLayer extends import_core10.CompositeLayer {
  initializeState() {
  ...
  _calculateH3DataProps() {
    ...
    const { iterable, objectInfo } = (0, import_core10.createIterable)(this.props.data);
    for (const object of iterable) {
      objectInfo.index++;
...
var H3ClusterLayer = class extends GeoCellLayer_default {
  ...
  updateState({ props, changeFlags }) {
    if (changeFlags.dataChanged || changeFlags.updateTriggersChanged && changeFlags.updateTriggersChanged.getHexagons) {
      ...
      const polygons = [];
      const { iterable, objectInfo } = (0, import_core11.createIterable)(data);
      for (const object of iterable) {
        objectInfo.index++;
...
var uniformBlock = `layout(std140) uniform meshUniforms {
  bool pickFeatureIds;
} mesh;
`;
var source = (
...
var source2 = import_shadertools.pbrMaterial.source.replace(/fn pbr_setPositionNormalTangentUV\([\s\S]*?\n}\n/, `fn pbr_setPositionNormalTangentUV(position: vec4f, normal: vec4f, tangent: vec4f, uv: vec2f)
{
...
var meshPbrMaterial = {
  ...import_shadertools.pbrMaterial,
  source: source2
};
...
var Tile3DLayer = class extends import_core12.CompositeLayer {
  initializeState() {
    if ("onTileLoadFail" in this.props) {
      import_core12.log.removed("onTileLoadFail", "onTileError")();
    }
    this.state = {
      layerMap: {},
      tileset3d: null,
  ...
  async _loadTileset(tilesetUrl) {
    ...
    const tilesetJson = await (0, import_core13.load)(actualTilesetUrl, loader, options.loadOptions);
  ...
  _makePointCloudLayer(tileHeader, oldLayer) {
    ...
    return new SubLayerClass({
      ...
      tile: tileHeader,
      data,
      coordinateSystem: import_core12.COORDINATE_SYSTEM.METER_OFFSETS,
  ...
  _makeSimpleMeshLayer(tileHeader, oldLayer) {
    ...
    const { attributes, indices, modelMatrix, cartographicOrigin, coordinateSystem = import_core12.COORDINATE_SYSTEM.METER_OFFSETS, material, featureIds } = content;
...
var TerrainLayer = class extends import_core14.CompositeLayer {
  updateState({ props, oldProps }) {
    ...
    if (props.workerUrl) {
      import_core14.log.removed("workerUrl", "loadOptions.terrain.workerUrl")();
    }
  }
  ...
  getTiledTerrainData(tile) {
    ...
    const overlappedBounds = getOverlappedBounds(bounds, this.props.tileSize, viewport instanceof import_core14._GlobeViewport);
  ...
  renderSubLayers(props) {
    ...
    const { viewport } = this.context;
    const isGlobe = viewport instanceof import_core14._GlobeViewport;
    const boundingBox = (_a = mesh == null ? void 0 : mesh.header) == null ? void 0 : _a.boundingBox;
    const hasLngLatBounds = boundingBox && boundingBox.every(([x, y]) => x >= -MAX_LONGITUDE && x <= MAX_LONGITUDE && y >= -MAX_LATITUDE && y <= MAX_LATITUDE);
    const coordinateSystem = isGlobe && hasLngLatBounds ? import_core14.COORDINATE_SYSTEM.LNGLAT : import_core14.COORDINATE_SYSTEM.CARTESIAN;
...
function Point([pointX, pointY], [nw, se], viewport) {
  const x = (0, import_core15.lerp)(nw[0], se[0], pointX);
  const y = (0, import_core15.lerp)(nw[1], se[1], pointY);
...
var MVTLayer = class extends tile_layer_default {
  ...
  renderSubLayers(props) {
    ...
    const modelMatrix = new import_core17.Matrix4().scale([xScale, yScale, 1]);
    props.autoHighlight = false;
    if (!this.context.viewport.resolution) {
      props.modelMatrix = modelMatrix;
      props.coordinateOrigin = [xOffset, yOffset, 0];
      props.coordinateSystem = import_core16.COORDINATE_SYSTEM.CARTESIAN;
    ...
    if (this.state.binary && !(subLayers instanceof import_layers8.GeoJsonLayer)) {
      import_core16.log.warn("renderSubLayers() must return GeoJsonLayer when using binary:true")();
    }
    return subLayers;
  }
  _updateAutoHighlight(info) {
