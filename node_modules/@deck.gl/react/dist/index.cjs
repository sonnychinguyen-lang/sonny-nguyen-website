...
function extractJSXLayers({ children, layers = [], views }) {
  ...
  const jsxLayers = [];
  const jsxViews = {};
  React2.Children.forEach(wrapInView(children), (reactElement) => {
...
function deckSizeMatchesContainer(thisRef, container) {
  const deck = thisRef.deck;
  return Boolean(deck && container && deck.width === container.clientWidth && deck.height === container.clientHeight);
}
function createDeckInstance(thisRef, DeckClass, props) {
  const deck = new DeckClass({
    ...
    _customRender: (redrawReason) => {
      ...
      const isWebGPU = ((_a = deck.device) == null ? void 0 : _a.type) === "webgpu";
...
var DeckGL = React3.forwardRef(DeckGLWithRef);
