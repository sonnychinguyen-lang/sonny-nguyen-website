...
export class ToggleWidget extends Widget {
    ...
    onRenderHTML(rootElement) {
        const { className, style, icon, label, tooltip, color, onIcon = icon, onLabel = label, onTooltip, onColor = color } = this.props;
        const on = this.checked;
        rootElement.dataset.checked = String(on);
        render(_jsx(IconButton, { className: className, style: style, icon: on ? onIcon : icon, label: on ? onLabel : label, tooltip: on ? onTooltip : tooltip, color: on ? onColor : color, onClick: this._toggle }), rootElement);
    }
}
