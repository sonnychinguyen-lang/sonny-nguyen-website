 ...
 * or filter.
 */
export async function meshoptDecodeVertexBuffer(target, count, byteStride, source, filter = 'NONE') {
    await MeshoptDecoder.ready;
    MeshoptDecoder.decodeVertexBuffer(target, count, byteStride, source, getMeshoptCompressionFilter(filter));
}
/**
 ...
 * @throws If the compressed bitstream is malformed or incompatible with the supplied dimensions.
 */
export async function meshoptDecodeIndexBuffer(target, count, byteStride, source) {
    await MeshoptDecoder.ready;
 ...
 * @throws If the compressed bitstream is malformed or incompatible with the supplied dimensions.
 */
export async function meshoptDecodeIndexSequence(target, count, byteStride, source) {
    await MeshoptDecoder.ready;
 ...
 */
// Keep this positional bridge aligned with MeshoptDecoder.decodeGltfBuffer.
// eslint-disable-next-line max-params
export async function meshoptDecodeGltfBuffer(target, count, byteStride, source, mode, filter = 'NONE') {
    await MeshoptDecoder.ready;
    MeshoptDecoder.decodeGltfBuffer(target, count, byteStride, source, mode, getMeshoptCompressionFilter(filter));
}
