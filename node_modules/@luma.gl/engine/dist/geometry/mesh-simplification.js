 ...
 */
export function simplifyMesh(options) {
    ...
    const targetIndexCount = Math.max(0, Math.min(indices.length, Math.floor((options.targetIndexCount ?? indices.length * (options.targetRatio ?? 0.5)) / 3) *
        3));
    ...
    const enqueueEdge = (firstVertex, secondVertex) => {
        ...
        for (const [retainedVertex, removedVertex] of [
            ...
            const geometricCost = Math.max(0, evaluateCombinedQuadric(vertices, positions, firstVertex, secondVertex, retainedVertex));
            queue.add({
                retainedVertex,
                removedVertex,
...
function addTriangleQuadric(vertices, positions, firstVertex, secondVertex, thirdVertex) {
    ...
    const planeOffset = -(planeX * positions[firstVertex * 3] +
...
function getAttributeCost(attributes, firstVertex, secondVertex) {
    ...
    for (const attribute of attributes) {
        ...
        const discrete = attribute.values instanceof Uint8Array ||
        ...
        for (let componentIndex = 0; componentIndex < attribute.size; componentIndex++) {
            const difference = attribute.values[firstVertex * attribute.size + componentIndex] -
...
function canCollapseBoundary(vertices, positions, retainedVertex, removedVertex) {
    ...
    const dot = retainedOffset[0] * otherOffset[0] +
...
function preservesTriangleOrientation(triangles, vertices, positions, retainedVertex, removedVertex) {
    for (const triangleIndex of vertices[removedVertex].triangles) {
        ...
        const replacement = triangle.vertices.map(vertex => vertex === removedVertex ? retainedVertex : vertex);
        const replacementNormal = getTriangleNormal(positions, ...replacement);
        const normalAlignment = originalNormal[0] * replacementNormal[0] +
...
class EdgeCollapseQueue {
    ...
    remove() {
        ...
        while (true) {
            ...
            const child = secondChild < this.entries.length &&
...
//# sourceMappingURL=mesh-simplification.js.map