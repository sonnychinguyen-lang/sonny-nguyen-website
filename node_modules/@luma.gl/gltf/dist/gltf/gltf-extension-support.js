...
export function getRegisteredGLTFExtensions() {
    return Object.entries(GLTF_EXTENSION_SUPPORT_REGISTRY)
        .sort(([leftExtensionName], [rightExtensionName]) => leftExtensionName.localeCompare(rightExtensionName))
...
export function getGLTFExtensionSupportSummary() {
    const summary = {
        total: 0,
        supported: 0,
...
export function getGLTFExtensionSupport(gltf, device) {
...
export function getUnsupportedRequiredGLTFExtensions(gltf, device) {
...
export function assertSupportedGLTFExtensions(gltf, device) {
    const unsupportedExtensions = getUnsupportedRequiredGLTFExtensions(gltf, device);
    if (unsupportedExtensions.length) {
        throw new Error(`Unsupported required glTF extensions: ${unsupportedExtensions
...
function getDeviceExtensionSupport(extensionName, definition, gltf, device) {
    ...
    return {
        supportLevel: 'none',
        comment: unsupportedFormat === null
...
export function getRegisteredGLTFExtensionSupport(extensionName) {
...
function collectGLTFExtensionNames(gltf) {
    ...
    if (gltfWithRemovedExtensions.lights?.length ||
...
//# sourceMappingURL=gltf-extension-support.js.map