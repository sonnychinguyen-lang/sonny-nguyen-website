...
import { getRegisteredGLTFExtensionSupport } from "../gltf/gltf-extension-support.js";
import { resolveTextureTransform, resolveTextureTransformSlot } from "../pbr/texture-transform.js";
        ...
        const channels = animation.channels.flatMap(({ sampler, target }) => {
            ...
            if (!parsedSampler) {
                ...
                parsedSampler = {
                    input: keyframeTimes,
                    interpolation,
                    output: morphTargetCount !== undefined
                        ? groupMorphTargetValues(keyframeValues, keyframeTimes.length, interpolation, morphTargetCount)
                        : keyframeValues
                };
...
function parseCameraPointerAnimationChannel(gltf, pointerSegments, sampler, pointer) {
    ...
    if (pointerSegments.length !== 4 ||
        !Number.isInteger(cameraIndex) ||
        !camera ||
        (projection !== 'perspective' && projection !== 'orthographic') ||
        camera.type !== projection ||
        !(projection === 'perspective' ? perspectiveProperties : orthographicProperties).includes(property)) {
...
function parseLightPointerAnimationChannel(gltf, pointerSegments, sampler, pointer) {
    ...
    const lightDefinitions = gltf.lights ||
...
function parseNodePointerAnimationChannel(gltf, pointerSegments, sampler, pointer) {
    const isVisibilityPointer = pointerSegments.length === 5 &&
    ...
    if (!Number.isInteger(nodeIndex) || !targetNode) {
        log.warn(`KHR_animation_pointer target ${pointer} references a missing node and will be skipped`)();
        return null;
    }
...
function groupMorphTargetValues(values, keyframeCount, interpolation, declaredTargetCount) {
    ...
    const targetCount = declaredTargetCount > 1
        ? declaredTargetCount
...
function parseMaterialPointerAnimationChannel(gltf, pointerSegments, sampler, pointer) {
    ...
    if (!Number.isInteger(materialIndex) || !material) {
        log.warn(`KHR_animation_pointer target ${pointer} references a missing material and will be skipped`)();
        return null;
    }
...
function resolveTextureTransformAnimationTarget(material, pointerSegments) {
    ...
    if (textureTransformPath !== 'offset' &&
