...
import { getTextureTransformMatrix, getTextureTransformSlotDefinition, getTextureTransformSlotDefinitions, resolveTextureCoordinateSet, resolveTextureTransform } from "../pbr/texture-transform.js";
...
function warnOnMissingExpectedAttributes(material, attributes) {
    ...
    if (uv1DependentTextureSlots.length > 0 && !hasGLTFAttribute(attributes, 'TEXCOORD_1')) {
        log.warn(`glTF material uses ${uv1DependentTextureSlots.join(', ')} with TEXCOORD_1 but primitive is missing TEXCOORD_1; those textures will be skipped`)();
    }
...
function addTexture(device, gltfTexture, uniformName, parsedMaterial, textureParseOptions = {}) {
    ...
    if (textureCoordinateSet > 1) {
        log.warn(`Skipping ${String(uniformName)} because ${textureCoordinateSet} is not supported; only TEXCOORD_0 and TEXCOORD_1 are currently available`)();
        return;
    }
    if (textureCoordinateSet === 1 && !hasGLTFAttribute(attributes, 'TEXCOORD_1')) {
        log.warn(`Skipping ${String(uniformName)} because it requires TEXCOORD_1 but the primitive does not provide TEXCOORD_1`)();
        return;
    }
    ...
    if (textureTransformSlot) {
        ...
        parsedMaterial.uniforms[textureTransformSlotDefinition.uvSetUniform] =
...
export function createGLTFTexture(device, image, options) {
...
export function createCompressedTexture(device, image, baseOptions) {
    ...
    if (!device.isTextureFormatSupported(format)) {
        log.warn(`createCompressedTexture: ${device.type} device does not support '${format}', creating fallback`)();
        return createCompressedTextureFallback(device, baseOptions);
    }
