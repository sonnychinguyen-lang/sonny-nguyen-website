...
const TEXTURE_TRANSFORM_SLOT_DEFINITIONS = [
...
const TEXTURE_TRANSFORM_SLOT_DEFINITION_MAP = new Map(TEXTURE_TRANSFORM_SLOT_DEFINITIONS.map(definition => [definition.slot, definition]));
...
export function getTextureTransformSlotDefinitions() {
    return TEXTURE_TRANSFORM_SLOT_DEFINITIONS;
}
export function getTextureTransformSlotDefinition(slot) {
...
export function getDefaultTextureTransform() {
    return {
        offset: [...IDENTITY_TEXTURE_TRANSFORM.offset],
        rotation: IDENTITY_TEXTURE_TRANSFORM.rotation,
        scale: [...IDENTITY_TEXTURE_TRANSFORM.scale]
    };
}
export function resolveTextureTransform(textureInfo) {
...
export function resolveTextureCoordinateSet(textureInfo) {
    const extensionTextureTransform = textureInfo?.['extensions']?.['KHR_texture_transform'];
    return extensionTextureTransform?.['texCoord'] ?? textureInfo?.['texCoord'] ?? 0;
}
export function resolveTextureTransformSlot(pointerSegments) {
    return (TEXTURE_TRANSFORM_SLOT_DEFINITIONS.find(definition => definition.pathSegments.length === pointerSegments.length &&
        definition.pathSegments.every((segment, index) => pointerSegments[index] === segment)) || null);
}
export function getTextureTransformMatrix(transform) {
...
export function getTextureTransformDeltaMatrix(baseTransform, currentTransform) {
