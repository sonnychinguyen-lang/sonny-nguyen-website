...
import { createTransientView, doGraphDataViewsOverlap, getViewBinding, getViewElementOffset, validatePackedUint32View, validatePackedView } from "./graph-data-view-utils.js";
...
export class GPUBVHQuery {
    ...
    constructor(props) {
        ...
        if (this.visitedCount)
            validatePackedUint32View(this.visitedCount, `${this.id} visitedCount`);
    ...
    addToGraph(graph) {
        ...
        const activeNodes = createTransientView(graph, `${this.id}-active-nodes`, 'uint32', this.nodeCount);
        ...
        if (this.outputMask && this.output.length > 0)
            addOutputMaskPass(graph, this);
    }
}
function validateIndexView(query) {
    ...
    if (query.bvh.nodeMinima.format !== query.bvh.nodeMaxima.format ||
...
function addInitializePass(graph, query, activeNodes) {
  ...
  if (index == 0u) {
    ...
    const resources = [
        ...
        ...(query.outputMask
            ...
            : []),
        ...(query.visitedCount
            ? [{ buffer: query.visitedCount, usage: 'storage-write' }]
            : [])
    ];
    addComputationPass(graph, {
        ...
        dispatchCount: Math.ceil(Math.max(query.nodeCount, query.outputMask?.length ?? 0, 1) / BVH_QUERY_WORKGROUP_SIZE)
    });
}
function addTraversalLevelPass(graph, query, activeNodes, depth) {
  ...
  if (selected) {
    ...
    addComputationPass(graph, {
        ...
        resources: [
            ...
            { buffer: query.query, usage: 'storage-read' },
            ...(query.visitedCount
                ? [{ buffer: query.visitedCount, usage: 'storage-read-write' }]
                : [])
        ],
        bindings: {
            activeNodes,
...
function validateDisjointViews(query) {
    const inputs = [
...
function addOutputMaskPass(graph, query) {
    const source = /* wgsl */ `
const OUTPUT_CAPACITY: u32 = ${query.output.length}u;
const MASK_LENGTH: u32 = ${query.outputMask.length}u;
const OUTPUT_OFFSET: u32 = ${getViewElementOffset(query.output)}u;
...
function makeNodePredicate(query) {
    const axes = ['X', 'Y', ...(query.dimension === 3 ? ['Z'] : [])];
    const nodeValues = axes
        .map((axis, axisIndex) => `let nodeMin${axis} = nodeMinima[NODE_MINIMA_OFFSET + nodeIndex * DIMENSION + ${axisIndex}u];
  let nodeMax${axis} = nodeMaxima[NODE_MAXIMA_OFFSET + nodeIndex * DIMENSION + ${axisIndex}u];`)
        .join('\n  ');
    const validNode = axes
        .map(axis => `finite(nodeMin${axis}) && finite(nodeMax${axis}) && nodeMin${axis} <= nodeMax${axis}`)
        .join(' && ');
    if (query.kind === 'point') {
  ...
  let selected = ${validNode} && ${validQuery} && ${contains};`;
    ...
    const queryValues = axes
        .map((axis, axisIndex) => `let queryMin${axis} = queryValues[QUERY_OFFSET + ${axisIndex}u];
  let queryMax${axis} = queryValues[QUERY_OFFSET + ${axisIndex + query.dimension}u];`)
        .join('\n  ');
    const validQuery = axes
        .map(axis => `finite(queryMin${axis}) && finite(queryMax${axis}) && queryMin${axis} <= queryMax${axis}`)
        .join(' && ');
...
//# sourceMappingURL=gpu-bvh-query.js.map