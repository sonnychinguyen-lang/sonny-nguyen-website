...
export class GPUBVH {
    ...
    constructor(props) {
        ...
        const limits = this.minima.buffer.graph.device.limits;
        const supportsFusedStrategy = this.leafCapacity <= MAXIMUM_FUSED_LEAF_CAPACITY &&
        ...
        this.stats = {
            ...
            outputByteLength: this.nodeCount * (boundsByteLength * 2 + Uint32Array.BYTES_PER_ELEMENT * 2) +
    ...
    addToGraph(graph) {
        ...
        if (this.resolvedStrategy === 'fused') {
            ...
            addLoadLeavesPass(graph, this, getGPUBVHDispatchLayout(this.nodeCount, graph.device.limits.maxComputeWorkgroupsPerDimension));
...
function addFusedRefitPass(graph, bvh) {
    const source = /* wgsl */ `
...
function addLoadLeavesPass(graph, bvh, dispatchLayout) {
    const source = /* wgsl */ `
const SOURCE_COUNT: u32 = ${bvh.minima.length}u;
const STORED_COUNT: u32 = ${Math.min(bvh.minima.length, bvh.leafCapacity)}u;
const LEAF_CAPACITY: u32 = ${bvh.leafCapacity}u;
...
function addRemapSourceIdsPass(graph, bvh, sourceIds) {
    const storedCount = Math.min(sourceIds.length, bvh.leafCapacity);
    const dispatchLayout = getGPUBVHDispatchLayout(storedCount, graph.device.limits.maxComputeWorkgroupsPerDimension);
    const source = /* wgsl */ `
...
function addRefitLevelPass(graph, bvh, depth) {
...
export function getGPUBVHDispatchLayout(nodeCount, maxComputeWorkgroupsPerDimension) {
    ...
    if (z > maximum) {
        throw new Error(`GPUBVH requires ${workgroupCount} workgroups, exceeding the 3D dispatch limit of ${maximum} per dimension`);
    }
    return { x, y, z };
}
...
//# sourceMappingURL=gpu-bvh.js.map