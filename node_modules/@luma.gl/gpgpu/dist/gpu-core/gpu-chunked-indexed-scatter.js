...
export class GPUChunkedIndexedScatter {
    ...
    addToGraph(graph) {
        ...
        const chunkState = createTransientView(graph, `${this.id}-chunk-state`, 'uint32', chunkCount * 3);
        const chunkCounts = graph.createDataView(chunkState.buffer, {
            format: 'uint32',
            length: chunkCount
        });
        const chunkOffsets = graph.createDataView(chunkState.buffer, {
            format: 'uint32',
            length: chunkCount,
            byteOffset: chunkCount * UINT32_BYTE_LENGTH
        });
        const dispatchCommands = createTransientView(graph, `${this.id}-dispatch-commands`, 'uint32', chunkCount * 3, Buffer.STORAGE | Buffer.INDIRECT);
        const sourceDispatchCommand = createTransientView(graph, `${this.id}-source-dispatch-command`, 'uint32', 3, Buffer.STORAGE | Buffer.INDIRECT);
...
function validateRouteLayout(id, layout) {
...
function validateChunkEnds(id, chunkEnds) {
...
function getRouteDeclarations(scatter) {
...
function addInitializePass(graph, scatter, chunkState, sourceDispatchCommand) {
    const source = /* wgsl */ `
...
function addCountPass(graph, scatter, chunkState, sourceDispatchCommand) {
    const useSubgroups = scatter.chunkEnds.length <= MAXIMUM_ROUTE_COUNT &&
...
${getSubgroupCoalescedAtomicAddWGSL('chunkIndex != INVALID_CHUNK_INDEX', 'chunkIndex', 'localChunkCounts', scatter.chunkEnds.length)}
  }`
        : `  let count = min(sourceCount[SOURCE_COUNT_OFFSET], SOURCE_CAPACITY);
  let workgroupIndex =
    (workgroupId.z * workgroupCount.y + workgroupId.y) * workgroupCount.x + workgroupId.x;
...
function addPublishPass(graph, scatter, chunkState, dispatchCommands) {
...
function addScatterPass(graph, scatter, chunkState, sourceDispatchCommand) {
    const chunkCount = scatter.chunkEnds.length;
    const useSubgroups = chunkCount <= MAXIMUM_ROUTE_COUNT && getGPUShaderSubgroupStrategy(graph.device) === 'subgroups';
    const source = /* wgsl */ `
${useSubgroups ? 'enable subgroups;' : ''}
${getRouteDeclarations(scatter)}
const CURSOR_BASE: u32 = ${chunkCount * 2}u;
...
//# sourceMappingURL=gpu-chunked-indexed-scatter.js.map