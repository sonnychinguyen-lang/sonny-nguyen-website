 ...
 * @internal
 */
export function compileGPUCommandGraph(props) {
    ...
    const transientTextures = new Map();
    const compiledNodes = [];
    try {
    ...
    const stats = {
        ...
        textureReusePercentage: logicalTransientTextureBytes > 0
...
function getGPUCommandGraphPreflightReport(device, buffers, nodes) {
    const nodeReports = nodes.map(node => {
        ...
        return Object.freeze({
            ...
            ...(node.condition
                ? {
                    condition: Object.freeze(node.condition.source === 'cpu'
                        ? { id: node.condition.id, source: 'cpu', mode: 'skip' }
                        : {
    ...
    return Object.freeze({
        ...
        conditionalNodeCount: nodes.filter(node => node.condition !== undefined).length,
        ...
        fitsDeviceLimits: largestBufferByteLength <= device.limits.maxBufferSize &&
...
export function getBufferHandle(buffer) {
 ...
 * @internal
 */
export function validateGraphNodeBufferBindingAliasing(node) {
    if (node.type === 'copy')
        return;
    const storageUses = (node.resources ?? []).filter((resource) => isGraphBufferUse(resource) &&
        (resource.usage === 'storage-read' ||
            resource.usage === 'storage-write' ||
            resource.usage === 'storage-read-write'));
    for (let firstIndex = 0; firstIndex < storageUses.length; firstIndex++) {
        ...
        for (let secondIndex = firstIndex + 1; secondIndex < storageUses.length; secondIndex++) {
            ...
            if (first.buffer === second.buffer ||
                getBufferHandle(first.buffer) !== getBufferHandle(second.buffer) ||
                (!isBufferWriteUsage(first.usage) && !isBufferWriteUsage(second.usage))) {
                continue;
            }
            const firstRange = getStorageBindingRange(first.buffer);
            const secondRange = getStorageBindingRange(second.buffer);
            if (firstRange.offset < secondRange.offset + secondRange.size &&
                secondRange.offset < firstRange.offset + firstRange.size) {
...
function getStorageBindingRange(buffer) {
    ...
    const viewByteLength = buffer.length === 0
        ? buffer.rowByteLength
        : (buffer.length - 1) * buffer.byteStride + buffer.rowByteLength;
...
function getNodeOrder(nodes) {
    ...
    const lastBufferWriter = new Map();
    const activeBufferReaders = new Map();
        ...
        for (const resource of node.resources ?? []) {
            if (isGraphBufferUse(resource)) {
                ...
                if (isBufferReadUsage(resource.usage)) {
                    ...
                    const readers = activeBufferReaders.get(handle) ?? new Set();
                    readers.add(node.id);
...
function textureUsesOverlap(left, right) {
    ...
    return (aspectsOverlap(leftRange.aspect, rightRange.aspect) &&
        intervalsOverlap(leftRange.baseMipLevel, leftRange.mipLevelCount, rightRange.baseMipLevel, rightRange.mipLevelCount) &&
        intervalsOverlap(leftRange.baseArrayLayer, leftRange.arrayLayerCount, rightRange.baseArrayLayer, rightRange.arrayLayerCount));
}
...
function getBufferTransientAllocationPlan(nodes, buffers) {
    ...
    const transientBuffers = Array.from(buffers)
        ...
        .sort((left, right) => (left.lifetime?.firstUse ?? Number.MAX_SAFE_INTEGER) -
...
function getTextureTransientAllocationPlan(nodes, textures) {
    ...
    for (const { texture, lifetime } of transientTextures) {
        ...
        let allocation = allocations.find(candidate => candidate.lastUse < lifetime.firstUse &&
...
function areTextureDescriptorsCompatible(descriptor, texture) {
    return (descriptor.format === texture.format &&
...
function getTextureByteLength(texture) {
    ...
    for (let mipLevel = 0; mipLevel < texture.mipLevels; mipLevel++) {
        byteLength = addSafeByteLengths(byteLength, textureFormatDecoder.computeMemoryLayout({
            format: texture.format,
...
//# sourceMappingURL=gpu-command-graph-compiler.js.map