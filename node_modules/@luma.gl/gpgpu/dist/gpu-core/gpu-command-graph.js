...
import { getGPUVectorFormatInfo, isValueListGPUVectorFormat, isVertexListGPUVectorFormat } from '@luma.gl/gpgpu/gpu-data';
import { compileGPUCommandGraph, getBufferHandle, getTextureHandle, isGraphBufferUse, isGraphTextureUse, validateGraphNodeBufferBindingAliasing } from "./gpu-command-graph-compiler.js";
 ...
 */
export class GPUCommandGraphExecution {
...
export function getGPUCommandGraphExecutionPlan(preflight, budget, options = {}) {
    ...
    while (nextNodeIndex < nodes.length) {
        ...
        while (nextNodeIndex < nodes.length) {
            ...
            if (nextNodeIndex > firstNodeIndex &&
                exceedsGPUCommandGraphExecutionBudget(nextCost, budget)) {
                break;
            }
            cost = nextCost;
            nextNodeIndex++;
        ...
        const publications = publicationPolicy === 'progressive'
            ? Object.freeze(nodes
                .slice(firstNodeIndex, nextNodeIndex)
...
function exceedsGPUCommandGraphExecutionBudget(cost, budget) {
    return (cost.maximumInvocationCount > budget.maximumInvocationCount ||
        (budget.maximumNodeCount !== undefined && cost.nodeCount > budget.maximumNodeCount) ||
        (budget.maximumCommandCount !== undefined && cost.commandCount > budget.maximumCommandCount) ||
        (budget.maximumReadByteLength !== undefined &&
            cost.readByteLength > budget.maximumReadByteLength) ||
        (budget.maximumWriteByteLength !== undefined &&
            cost.writeByteLength > budget.maximumWriteByteLength));
}
...
/** Redirects one direct dispatch to a GPU-written indirect command without exposing a fake skip. */
...
export class GPUCommandGraphEncoding {
    ...
    nodes;
    /** @internal */
    constructor(nodes, cpuEncodeTimeMilliseconds, computePassCount = nodes.filter(node => node.stats.type === 'compute' && node.stats.condition?.outcome !== 'skipped').length) {
        this.nodes = nodes;
        this.canReadGPUTimings = nodes.some(node => node.timestamp !== undefined);
        const executedNodes = nodes.filter(node => node.stats.condition?.outcome !== 'skipped');
...
export class GPUCommandGraph {
    ...
    importFrameTexture(descriptor) {
        this.assertMutable();
        const normalizedDescriptor = normalizeGraphTextureDescriptor(descriptor, this.device);
        return this.addTexture(new GraphTextureHandle(this, normalizedDescriptor, false, undefined, true));
    }
    /**
        ...
        const attachmentUses = node.attachments
            ? [
                ...
                ...(node.attachments.resolveTargets ?? [])
                    ...
                    .map(texture => ({ texture, usage: 'render-attachment' })),
    ...
    addNode(node) {
        ...
        if (node.condition) {
            ...
            if (node.condition.source === 'cpu') {
                ...
                if (!(node.resources ?? []).some(resource => isGraphBufferUse(resource) &&
                    getBufferHandle(resource.buffer) === gpuCondition.buffer &&
    ...
    addTexture(texture) {
        if (this.buffers.has(texture.id) ||
    ...
    validateRenderAttachments(id, attachments) {
        ...
        for (const attachment of remaining) {
            if (attachment.width !== first.width ||
 ...
 */
export class CompiledGPUCommandGraph {
    ...
    externalTextures;
    compiledNodes;
    activeImportedBufferHandles = new Set();
    writableImportedBufferHandles = new Set();
    activeImportedTextureHandles = new Set();
    writableImportedTextureHandles = new Set();
    transientBuffers;
    transientTextures;
    ...
    cachedTextureViews = [];
    cachedFramebuffers = [];
    lastFrameIds = new Map();
    lastExternalTextureFrameIds = new Map();
    consumedExternalTextures = new WeakSet();
    destroyed = false;
            ...
            for (const resource of node.resources ?? []) {
                if (isGraphBufferUse(resource)) {
                    ...
                    if (!handle.transient) {
                        ...
                        if (resource.usage === 'storage-write' ||
    ...
    createExecution(budget, options = {}) {
        return new GPUCommandGraphExecution({
            ...
            budget,
            encodeNodeRange: (commandEncoder, options, firstNodeIndex, nextNodeIndex) => this.encodeNodeRange(commandEncoder, options, firstNodeIndex, nextNodeIndex)
        });
    }
    encodeNodeRange(commandEncoder, options, firstNodeIndex, nextNodeIndex) {
        ...
        const getTextureView = (textureOrView) => {
            ...
            const cached = this.cachedTextureViews.find(entry => entry.logicalView === textureOrView &&
            ...
            for (let nodeIndex = firstNodeIndex; nodeIndex < nextNodeIndex; nodeIndex++) {
                ...
                switch (node.type) {
                    case 'compute': {
                        ...
                        try {
                            const gpuCondition = node.condition?.source === 'gpu' ? node.condition : undefined;
                            const conditionalPass = gpuCondition
                                ? makeGPUIndirectConditionalComputePass(computePass, getBuffer(gpuCondition.buffer), gpuCondition.byteOffset ?? 0, node.id)
                                : undefined;
                            executable.encode({
                                ...baseContext,
                                computePass: conditionalPass?.computePass ?? computePass
                            });
                            conditionalPass?.assertDispatched();
                        }
                        finally {
                    ...
                    case 'render': {
                        ...
                        const framebuffer = node.attachments
                            ...
                            : undefined;
                        const resolveTargets = node.attachments?.resolveTargets?.map(target => target ? getTextureView(target) : null);
                        ...
                        ...(node.condition
                            ? {
                                condition: {
                                    ...
                                    outcome: node.condition.source === 'gpu'
                                        ? 'gpu-resolved'
                                        : 'executed'
                                }
                            }
    ...
    resolveImportedBuffers(overrides) {
        ...
        const activePhysicalBuffers = new Map();
        for (const [id, handle] of this.buffers) {
            ...
            if (this.activeImportedBufferHandles.has(handle)) {
                ...
                const physicalBuffer = (typeof physicalHandle === 'object' && physicalHandle !== null) ||
    ...
    getFramebuffer(nodeId, attachments, getTextureView) {
        ...
        const depthStencilAttachment = attachments.depthStencilAttachment
            ...
            : undefined;
        const cached = this.cachedFramebuffers.find(entry => entry.nodeId === nodeId &&
    ...
    destroyFramebuffersUsingView(view) {
        for (let index = this.cachedFramebuffers.length - 1; index >= 0; index--) {
            ...
            if (cached.depthStencilAttachment === view ||
...
function getGPUCommandGraphCapabilities(device) {
    return Object.freeze({
        ...
        softwareAdapter: device.info.gpu === 'software' ||
...
function validateGraphExternalTextureDescriptor(descriptor, device) {
    ...
    if (descriptor.width > device.limits.maxTextureDimension2D ||
...
function normalizeGraphTextureDescriptor(descriptor, device) {
    ...
    const maximumWidth = dimension === '1d'
    ...
    if (descriptor.width > maximumWidth ||
...
function validateTextureViewForUsage(textureOrView, usage) {
    if (textureOrView instanceof GraphTextureView &&
...
//# sourceMappingURL=gpu-command-graph.js.map