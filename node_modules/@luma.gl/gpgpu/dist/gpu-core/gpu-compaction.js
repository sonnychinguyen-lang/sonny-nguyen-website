...
import { createTransientVectorView, createTransientView, getViewBinding, getViewElementOffset, validateMatchingVectorTopology, validatePackedUint32View } from "./graph-data-view-utils.js";
...
export class GPUCompaction {
    ...
    constructor(props) {
        ...
        if (this.input instanceof GraphVectorView &&
     ...
     */
    addToGraph(graph) {
        addGPUCompactionToGraphWithDispatchLimit(this, graph, graph.device.limits.maxComputeWorkgroupsPerDimension);
    }
}
/** Adds stable scan and scatter passes with an explicit dispatch limit. @internal */
export function addGPUCompactionToGraphWithDispatchLimit(compaction, graph, maxComputeWorkgroupsPerDimension) {
    for (const view of [
    ...
    const offsets = compaction.flags instanceof GraphVectorView
    ...
    addScatterPasses(graph, compaction.id, compaction.input, compaction.flags, offsets, compaction.output, compaction.count, maxComputeWorkgroupsPerDimension);
}
/** Writes the required zero count for an empty input. */
function addClearCountPass(graph, id, count) {
...
function addScatterPasses(graph, id, input, flags, offsets, output, count, maxComputeWorkgroupsPerDimension) {
    ...
    for (let outputChunkIndex = 0; outputChunkIndex < outputChunks.length; outputChunkIndex++) {
        ...
        if (outputChunk.length > 0) {
            for (const inputChunkIndex of inputChunkIndices) {
                const writesCount = inputChunkIndex === countInputChunkIndex && outputChunkIndex === countOutputChunkIndex;
                addScatterPass(graph, {
                    ...
                    dispatchLayout: getBoundedDispatchLayout('GPUCompaction', inputChunks[inputChunkIndex].length, COMPACTION_WORKGROUP_SIZE, maxComputeWorkgroupsPerDimension)
                });
            }
        }
  ...
  ${countWrite}
    ...
    addCompactionPass(graph, {
        ...
        resources: [
            ...
            ...(props.count ? [{ buffer: props.count, usage: 'storage-write' }] : [])
        ],
        bindings: {
...
function addCompactionPass(graph, props) {
    graph.addComputePass({
        id: props.id,
        workload: {
            ...
            readByteLength: props.resources.reduce((total, resource) => total +
                (resource.usage === 'storage-read' || resource.usage === 'storage-read-write'
                    ? resource.buffer.length * resource.buffer.rowByteLength
                    : 0), 0),
            writeByteLength: props.resources.reduce((total, resource) => total +
