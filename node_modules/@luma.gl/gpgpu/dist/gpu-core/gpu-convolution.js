...
import { getBoundedDispatchLayout, getBoundedInvocationIndexSource } from "./gpu-dispatch-utils.js";
import { GPU_FFT_COMMON_SHADER_SOURCE, GPU_FFT_MAX_LENGTH, makeGPUFFTPassPlan } from "./gpu-fft-utils.js";
import { createTransientView, getViewBinding, getViewElementOffset, validatePackedView } from "./graph-data-view-utils.js";
...
export function makeGPUConvolutionStats(props) {
...
export function getGPUConvolutionSupport(device, props) {
    let stats;
    try {
    ...
    if (device.limits.maxComputeInvocationsPerWorkgroup < GPU_CONVOLUTION_WORKGROUP_SIZE ||
    ...
    const largestScalarByteLength = Math.max(stats.elementCount, stats.kernelElementCount) * Float32Array.BYTES_PER_ELEMENT;
    if (largestScalarByteLength > device.limits.maxStorageBufferBindingSize ||
    ...
    try {
        getBoundedDispatchLayout('GPUConvolution', stats.elementCount, GPU_CONVOLUTION_WORKGROUP_SIZE, device.limits.maxComputeWorkgroupsPerDimension);
    }
    catch (error) {
        return { supported: false, reason: error.message, stats };
    }
...
function addGPUConvolutionDirectPass(graph, convolution) {
    const dispatchLayout = getBoundedDispatchLayout(convolution.id, convolution.stats.elementCount, GPU_CONVOLUTION_WORKGROUP_SIZE, graph.device.limits.maxComputeWorkgroupsPerDimension);
...
function addGPUConvolutionFFTPipeline(graph, convolution) {
    const { fftElementCount } = convolution.stats;
    const packedInput = createTransientView(graph, `${convolution.id}-fft-packed-input`, 'float32x2', fftElementCount);
    const packedKernel = createTransientView(graph, `${convolution.id}-fft-packed-kernel`, 'float32x2', fftElementCount);
    const inputSpectrum = createTransientView(graph, `${convolution.id}-fft-input-spectrum`, 'float32x2', fftElementCount);
    const kernelSpectrum = createTransientView(graph, `${convolution.id}-fft-kernel-spectrum`, 'float32x2', fftElementCount);
    const productSpectrum = createTransientView(graph, `${convolution.id}-fft-product-spectrum`, 'float32x2', fftElementCount);
    const inverseSpatial = createTransientView(graph, `${convolution.id}-fft-inverse-spatial`, 'float32x2', fftElementCount);
    const inputScratch = createTransientView(graph, `${convolution.id}-fft-input-scratch`, 'float32x2', fftElementCount);
    const kernelScratch = createTransientView(graph, `${convolution.id}-fft-kernel-scratch`, 'float32x2', fftElementCount);
    const inverseScratch = createTransientView(graph, `${convolution.id}-fft-inverse-scratch`, 'float32x2', fftElementCount);
    addGPUConvolutionPackPass(graph, convolution, packedInput, packedKernel);
    addGPUConvolutionFFT2D(graph, `${convolution.id}-fft-input`, packedInput, inputSpectrum, inputScratch, convolution.stats.fftWidth, convolution.stats.fftHeight, 'forward');
    addGPUConvolutionFFT2D(graph, `${convolution.id}-fft-kernel`, packedKernel, kernelSpectrum, kernelScratch, convolution.stats.fftWidth, convolution.stats.fftHeight, 'forward');
    addGPUConvolutionMultiplyPass(graph, convolution, inputSpectrum, kernelSpectrum, productSpectrum);
    addGPUConvolutionFFT2D(graph, `${convolution.id}-fft-inverse`, productSpectrum, inverseSpatial, inverseScratch, convolution.stats.fftWidth, convolution.stats.fftHeight, 'inverse');
    addGPUConvolutionCropPass(graph, convolution, inverseSpatial);
}
function addGPUConvolutionPackPass(graph, convolution, packedInput, packedKernel) {
    const dispatchLayout = getBoundedDispatchLayout(`${convolution.id} FFT pack`, convolution.stats.fftElementCount, GPU_CONVOLUTION_WORKGROUP_SIZE, graph.device.limits.maxComputeWorkgroupsPerDimension);
    ...
    addGPUConvolutionComputePass(graph, {
        ...
        readByteLength: (convolution.stats.elementCount + convolution.stats.kernelElementCount) *
            Float32Array.BYTES_PER_ELEMENT,
        writeByteLength: 2 * convolution.stats.fftComplexBufferByteLength
    });
}
function addGPUConvolutionMultiplyPass(graph, convolution, inputSpectrum, kernelSpectrum, productSpectrum) {
    const dispatchLayout = getBoundedDispatchLayout(`${convolution.id} FFT multiply`, convolution.stats.fftElementCount, GPU_CONVOLUTION_WORKGROUP_SIZE, graph.device.limits.maxComputeWorkgroupsPerDimension);
...
function addGPUConvolutionCropPass(graph, convolution, inverseSpatial) {
    const dispatchLayout = getBoundedDispatchLayout(`${convolution.id} FFT crop`, convolution.stats.elementCount, GPU_CONVOLUTION_WORKGROUP_SIZE, graph.device.limits.maxComputeWorkgroupsPerDimension);
...
function addGPUConvolutionFFT2D(graph, id, input, output, scratch, width, height, direction) {
    const passes = [
        ...makeGPUFFTPassPlan(width).map(pass => ({ axis: 'horizontal', ...pass })),
        ...makeGPUFFTPassPlan(height).map(pass => ({ axis: 'vertical', ...pass }))
    ];
    let passInput = input;
...
function addGPUConvolutionFFTPass(graph, props) {
    const elementCount = props.width * props.height;
    const dispatchLayout = getBoundedDispatchLayout(props.id, elementCount, GPU_CONVOLUTION_WORKGROUP_SIZE, graph.device.limits.maxComputeWorkgroupsPerDimension);
...
function addGPUConvolutionComputePass(graph, props) {
    ...
    graph.addComputePass({
        ...
        workload: {
            ...
            maximumInvocationCount: props.dispatchLayout.x *
        ...
        resources: [
            ...Object.values(props.bindings).map(buffer => ({ buffer, usage: 'storage-read' })),
            ...Object.values(props.outputs).map(buffer => ({ buffer, usage: 'storage-write' }))
        ],
        compile: ({ device }) => {
...
export function getGPUConvolutionDirectShaderSource(convolution, dispatchLayout) {
    const sampleSource = convolution.boundary === 'wrap'
...
export function getGPUConvolutionPackShaderSource(convolution, dispatchLayout) {
...
export function getGPUConvolutionFFTShaderSource(props, dispatchLayout) {
    const elementCount = props.width * props.height;
    const horizontal = props.pass.axis === 'horizontal';
    const directionSign = props.direction === 'forward' ? '-1.0' : '1.0';
    const normalizationScale = props.direction === 'inverse' && props.finalPass ? `${1 / elementCount}` : '1.0';
...
function validateGPUConvolutionDimensions(props) {
...
function getGPUConvolutionFFTDimension(inputDimension, kernelDimension, boundary) {
    const requiredDimension = boundary === 'zero' ? inputDimension + kernelDimension - 1 : inputDimension;
    return Math.max(2, 2 ** Math.ceil(Math.log2(requiredDimension)));
}
function getGPUConvolutionFFTReason(device, props, stats) {
    ...
    if (stats.fftComplexBufferByteLength > device.limits.maxStorageBufferBindingSize ||
    ...
    try {
        getBoundedDispatchLayout('GPUConvolution FFT', stats.fftElementCount, GPU_CONVOLUTION_WORKGROUP_SIZE, device.limits.maxComputeWorkgroupsPerDimension);
    }
    catch (error) {
...
function getComplexViewOffset(view) {
    return (view.byteOffset % 256) / (2 * Float32Array.BYTES_PER_ELEMENT);
}
function validateGPUConvolutionOwnership(graph, view, name) {
    if (view.buffer.graph !== graph) {
        throw new Error(`${name} belongs to a different GPUCommandGraph`);
    }
}
//# sourceMappingURL=gpu-convolution.js.map