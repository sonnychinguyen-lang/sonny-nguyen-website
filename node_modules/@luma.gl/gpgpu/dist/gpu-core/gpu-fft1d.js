...
import { getGPUFFTLengthReason, GPU_FFT_COMMON_SHADER_SOURCE, GPU_FFT_MAX_LENGTH, GPU_FFT_MIN_LENGTH, makeGPUFFTPassPlan } from "./gpu-fft-utils.js";
import { getGPUShaderSubgroupStrategy } from "./gpu-subgroup-utils.js";
import { createTransientView, getViewBinding } from "./graph-data-view-utils.js";
...
export class GPUFFT1D {
    ...
    addToGraph(graph) {
        ...
        const scratch = createTransientView(graph, `${this.id}-scratch`, 'float32x2', this.stats.elementCount);
        ...
        for (const [passIndex, pass] of passPlan.entries()) {
            ...
            const useSubgroups = support.strategy === 'subgroups' &&
                pass.kind === 'butterfly' &&
...
export function makeGPUFFT1DStats(length, batchCount = 1) {
...
export function getGPUFFT1DSupport(device, props) {
    let stats;
    try {
    ...
    try {
        getBoundedDispatchLayout('GPUFFT1D', stats.elementCount, GPU_FFT1D_WORKGROUP_SIZE, device.limits.maxComputeWorkgroupsPerDimension);
    }
    catch (error) {
    ...
    return {
        ...
        stats,
        strategy,
        subgroupStageCount: strategy === 'subgroups' ? getGPUFFT1DSubgroupStageCount(device, props.length) : 0
    };
}
/** Resolves an explicit or automatic subgroup preference. */
export function getGPUFFT1DStrategy(device, requestedStrategy = 'auto') {
    const subgroupSize = device.info?.subgroupMinSize;
    const subgroupsAvailable = getGPUShaderSubgroupStrategy(device, { requiresSubgroupId: true }) === 'subgroups' &&
...
function addGPUFFT1DPass(graph, props) {
    const elementCount = props.length * props.batchCount;
    const dispatchLayout = getBoundedDispatchLayout(props.id, elementCount, GPU_FFT1D_WORKGROUP_SIZE, graph.device.limits.maxComputeWorkgroupsPerDimension);
    ...
    graph.addComputePass({
        ...
        workload: {
            ...
            maximumInvocationCount: dispatchLayout.x * dispatchLayout.y * dispatchLayout.z * GPU_FFT1D_WORKGROUP_SIZE,
...
export function getGPUFFT1DShaderSource(props, dispatchLayout) {
    const directionSign = props.direction === 'forward' ? '-1.0' : '1.0';
    const normalizationScale = props.direction === 'inverse' && props.finalPass ? `${1 / props.length}` : '1.0';
...
@compute @workgroup_size(${GPU_FFT1D_WORKGROUP_SIZE}) fn main(
  @builtin(workgroup_id) workgroupId: vec3u,
  @builtin(local_invocation_index) localInvocationIndex: u32${props.useSubgroups
