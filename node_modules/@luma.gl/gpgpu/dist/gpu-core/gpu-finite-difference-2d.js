...
import { getViewBinding, getViewElementOffset, validatePackedView } from "./graph-data-view-utils.js";
...
export class GPUFiniteDifference2D {
    ...
    constructor(props) {
        ...
        validatePackedView(this.output, [props.operator === 'gradient' ? 'float32x2' : 'float32'], `${this.id} output`);
    ...
    addToGraph(graph) {
        ...
        const dispatchLayout = getBoundedDispatchLayout(this.id, this.stats.elementCount, GPU_FINITE_DIFFERENCE_2D_WORKGROUP_SIZE, graph.device.limits.maxComputeWorkgroupsPerDimension);
        ...
        graph.addComputePass({
            ...
            workload: {
                ...
                maximumInvocationCount: dispatchLayout.x *
                    dispatchLayout.y *
                    dispatchLayout.z *
...
export function makeGPUFiniteDifference2DStats(props) {
    ...
    return Object.freeze({
        ...
        height: props.height,
        elementCount,
        spacing: Object.freeze([props.spacing[0], props.spacing[1]]),
...
export function getGPUFiniteDifference2DSupport(device, props) {
    let stats;
    try {
    ...
    if (device.limits.maxComputeInvocationsPerWorkgroup < GPU_FINITE_DIFFERENCE_2D_WORKGROUP_SIZE ||
    ...
    const largestByteLength = stats.elementCount * Math.max(stats.inputComponentCount, stats.outputComponentCount) * 4;
    if (largestByteLength > device.limits.maxStorageBufferBindingSize ||
    ...
    try {
        getBoundedDispatchLayout('GPUFiniteDifference2D', stats.elementCount, GPU_FINITE_DIFFERENCE_2D_WORKGROUP_SIZE, device.limits.maxComputeWorkgroupsPerDimension);
    }
    catch (error) {
...
export function getGPUFiniteDifference2DShaderSource(difference, dispatchLayout) {
...
function getArrayElementOffset(view, componentCount, name) {
...
//# sourceMappingURL=gpu-finite-difference-2d.js.map