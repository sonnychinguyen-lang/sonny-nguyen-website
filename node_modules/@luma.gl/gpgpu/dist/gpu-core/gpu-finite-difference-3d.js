...
export class GPUFiniteDifference3D {
    ...
    constructor(props) {
        ...
        if (this.input.length < this.stats.elementCount ||
            ...
            throw new Error(`${this.id} input and output must contain at least width * height * depth rows`);
        }
    ...
    addToGraph(graph) {
        ...
        const dispatchLayout = getBoundedDispatchLayout(this.id, this.stats.elementCount, GPU_FINITE_DIFFERENCE_3D_WORKGROUP_SIZE, graph.device.limits.maxComputeWorkgroupsPerDimension);
...
export function makeGPUFiniteDifference3DStats(props) {
    ...
    return Object.freeze({
        ...
        depth: props.depth,
        elementCount,
        spacing: Object.freeze([...props.spacing]),
...
export function getGPUFiniteDifference3DSupport(device, props) {
    let stats;
    try {
    ...
    if (device.limits.maxComputeInvocationsPerWorkgroup < GPU_FINITE_DIFFERENCE_3D_WORKGROUP_SIZE ||
    ...
    try {
        getBoundedDispatchLayout('GPUFiniteDifference3D', stats.elementCount, GPU_FINITE_DIFFERENCE_3D_WORKGROUP_SIZE, device.limits.maxComputeWorkgroupsPerDimension);
    }
    catch (error) {
...
export function getGPUFiniteDifference3DShaderSource(difference, dispatchLayout) {
...
//# sourceMappingURL=gpu-finite-difference-3d.js.map