...
export class GPUGallopingSearch {
    ...
    constructor(props) {
        ...
        if (!Number.isSafeInteger(queriesPerTile) ||
            queriesPerTile < 1 ||
            queriesPerTile > MAXIMUM_QUERIES_PER_TILE) {
            throw new Error(`${this.id} queriesPerTile must be an integer from 1 through ${MAXIMUM_QUERIES_PER_TILE}`);
        }
        const sources = [
            this.values,
        ...
        if (sources.some(view => view.buffer === this.output.buffer) ||
...
function addValidationClearPass(graph, id, validationErrors) {
    const source = /* wgsl */ `
...
function addSearchPass(graph, search) {
    const tilesPerSegment = Math.ceil(search.stats.maximumQueryCount / search.stats.queriesPerTile);
    const dispatchLayout = getBoundedDispatchLayout(search.id, search.stats.maximumSearchCount, GALLOPING_SEARCH_WORKGROUP_SIZE, graph.device.limits.maxComputeWorkgroupsPerDimension);
  ...
  return values[VALUE_OFFSET + row * VALUE_STRIDE];`
    ...
    const finiteQuery = search.values.format === 'float32'
...
function addSearchComputationPass(graph, search, source, dispatchLayout) {
    const bindings = {
    ...
    const resources = [
        ...
        ...(search.valueOrder ? [{ buffer: search.valueOrder, usage: 'storage-read' }] : []),
    ...
    graph.addComputePass({
        ...
        workload: {
            ...
            maximumInvocationCount: dispatchLayout.x * dispatchLayout.y * dispatchLayout.z * GALLOPING_SEARCH_WORKGROUP_SIZE
        },
        compile: ({ device }) => {
            const computation = new Computation(device, {
                id: `${search.id}-query`,
...
function validateScalarValueView(view, name) {
    if ((view.format !== 'float32' && view.format !== 'uint32') ||
...
//# sourceMappingURL=gpu-galloping-search.js.map