...
import { createTransientView, getViewBinding, getViewElementOffset, validateMatchingVectorTopology, validatePackedView } from "./graph-data-view-utils.js";
import { getGPUShaderSubgroupStrategy, getSubgroupBallotHelpersWGSL } from "./gpu-subgroup-utils.js";
...
export class GPUGridAggregation {
    ...
    addToGraph(graph) {
        ...
        if (positionChunks.some(chunk => chunk.buffer.graph !== graph) ||
        ...
        const counts = this.operation === 'mean'
        ...
        for (let chunkIndex = 0; chunkIndex < positionChunks.length; chunkIndex++) {
            if (positionChunks[chunkIndex].length > 0) {
                addGridAggregationPass(graph, {
                    id: this.positions instanceof GraphVectorView ? `${this.id}-chunk-${chunkIndex}` : this.id,
...
function addInitializeGridAggregationPass(graph, id, output, operation, counts) {
...
function addGridAggregationPass(graph, aggregation) {
    const [width, height] = aggregation.gridSize;
    const useSubgroups = aggregation.output.length <= MAXIMUM_SUBGROUP_COALESCED_CELL_COUNT &&
        getGPUShaderSubgroupStrategy(graph.device) === 'subgroups';
    const gpuBounds = isGPUGridBoundsView(aggregation.bounds);
    const literalBounds = aggregation.bounds;
  ...
  let maximumY = ${getFloatLiteral(literalBounds[3])};`;
    const accumulation = useSubgroups
        ? getSubgroupGridAggregationWGSL(aggregation.operation, aggregation.output.length, aggregation.counts)
        : `  if (accepted) {
...
${gpuBounds ? `const BOUNDS_OFFSET: u32 = ${getViewElementOffset(aggregation.bounds)}u;` : ''}
...
${accumulation}
    ...
    const resources = [
        ...
        ...(gpuBounds
            ? [{ buffer: aggregation.bounds, usage: 'storage-read' }]
            : []),
        { buffer: aggregation.output, usage: 'storage-read-write' },
        ...(aggregation.counts
            ? [{ buffer: aggregation.counts, usage: 'storage-read-write' }]
            : [])
    ];
    addComputationPass(graph, {
        ...
        bindings: {
            ...
            ...(gpuBounds ? { boundsValues: aggregation.bounds } : {}),
...
function addFinalizeGridAggregationPass(graph, id, output, operation, counts) {
    const countBinding = counts
...
function getAggregationFunction(operation) {
...
function getAggregationCall(operation, cellIndex = 'cellIndex', valueExpression = 'weight') {
    if (operation === 'min' || operation === 'max') {
...
function getSubgroupGridAggregationWGSL(operation, cellCount, counts) {
...
function validateMatchingInputs(positions, weights, id) {
...
//# sourceMappingURL=gpu-grid-aggregation.js.map