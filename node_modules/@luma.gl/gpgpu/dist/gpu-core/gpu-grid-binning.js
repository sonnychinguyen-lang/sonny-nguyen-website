...
export class GPUGridBinning {
     ...
     */
    addToGraph(graph) {
        if (getPositionChunks(this.positions).some(chunk => chunk.buffer.graph !== graph) ||
...
function addClearGridPass(graph, id, output) {
...
function addGridPass(graph, binning) {
    ...
    const literalBounds = binning.bounds;
  ...
  let maximumY = ${getFloatLiteral(literalBounds[3])};`;
    const localAccumulation = useSubgroups
        ? getSubgroupCoalescedAtomicAddWGSL('accepted', 'cellIndex', 'localCounts', binning.output.length)
...
${gpuBounds ? `const BOUNDS_OFFSET: u32 = ${getViewElementOffset(binning.bounds)}u;` : ''}
...
@compute @workgroup_size(${GRID_WORKGROUP_SIZE}) fn main(
  ...
  ${accumulation}
    ...
    const resources = [
        ...
        ...(gpuBounds
            ? [{ buffer: binning.bounds, usage: 'storage-read' }]
            : []),
    ...
    addComputationPass(graph, {
        ...
        bindings: {
            ...
            ...(gpuBounds ? { boundsValues: binning.bounds } : {}),
...
//# sourceMappingURL=gpu-grid-binning.js.map