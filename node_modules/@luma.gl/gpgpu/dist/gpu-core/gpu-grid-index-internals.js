...
import { createTransientView, getViewBinding, getViewElementOffset } from "./graph-data-view-utils.js";
const GRID_INDEX_WORKGROUP_SIZE = 256;
/** Adds an index rebuild using an explicit device dispatch limit. @internal */
export function addGPUGridIndexToGraphWithDispatchLimit(index, graph, maxComputeWorkgroupsPerDimension) {
    const views = [
...
function addCountPass(graph, props) {
    const source = makePositionPassSource(props, 
    /* wgsl */ `
  if (accepted) { atomicAdd(&cellCounts[COUNTS_OFFSET + cellIndex], 1u); }`, false);
    addComputationPass(graph, {
        id: props.id,
        source,
        resources: [
...
function addScatterPass(graph, props) {
  ...
  if (accepted) {
    ...
    const resources = [
        ...
        ...(props.sourceIds
            ? [{ buffer: props.sourceIds, usage: 'storage-read' }]
            : [])
    ];
    addComputationPass(graph, {
        id: props.id,
...
function makePositionPassSource(props, body, scatter) {
    ...
    const zDeclarations = props.dimension === 3
    ...
    let inZ = z >= ${getFloatLiteral(props.bounds[2])} && z <= ${getFloatLiteral(props.bounds[5])};`
        : 'let finiteZ = true;\n    let inZ = true;';
    const zCoordinate = props.dimension === 3
...
export function getGPUGridIndexDispatchLayout(elementCount, maxComputeWorkgroupsPerDimension) {
...
export function getGPUGridIndexInvocationIndexSource(layout) {
...
//# sourceMappingURL=gpu-grid-index-internals.js.map