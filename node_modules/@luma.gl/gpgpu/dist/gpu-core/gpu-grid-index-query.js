...
import { doGraphDataViewsOverlap, getViewBinding, getViewElementOffset, validatePackedUint32View, validatePackedView } from "./graph-data-view-utils.js";
...
export class GPUGridIndexQuery {
    ...
    constructor(props) {
        ...
        const expectedQueryLength = this.kind === 'point'
            ? this.dimension
            : this.kind === 'bounds'
                ? this.dimension * 2
                : this.dimension + 1;
    ...
    addToGraph(graph) {
        ...
        if (this.index.objectIds.length > 0)
            addQueryPass(graph, this);
    }
}
function addInitializePass(graph, query) {
...
@compute @workgroup_size(${GRID_QUERY_WORKGROUP_SIZE}) fn main(
  ...
  ${maskClear}
    ...
    const resources = [
        ...
        ...(query.outputMask
            ? [{ buffer: query.outputMask, usage: 'storage-write' }]
            : [])
    ];
...
function addQueryPass(graph, query) {
...
function makeCellSelection(query) {
    const dimension = query.dimension;
    ...
    const cellDeclarations = axes
        .map(axis => `let cellMin${axis.name.toUpperCase()} = cellMinimum(${axis.coordinate}, ${axis.size}, ${getFloatLiteral(axis.minimum)}, ${getFloatLiteral(axis.maximum)});
  let cellMax${axis.name.toUpperCase()} = cellMaximum(${axis.coordinate}, ${axis.size}, ${getFloatLiteral(axis.minimum)}, ${getFloatLiteral(axis.maximum)});`)
    ...
    if (query.kind === 'point') {
        const values = axes
            .map((axis, axisIndex) => `let query${axis.name.toUpperCase()} = queryValues[QUERY_OFFSET + ${axisIndex}u];`)
            .join('\n  ');
        const valid = axes
            .map(axis => `finite(query${axis.name.toUpperCase()}) && query${axis.name.toUpperCase()} >= ${getFloatLiteral(axis.minimum)} && query${axis.name.toUpperCase()} <= ${getFloatLiteral(axis.maximum)}`)
            .join(' && ');
        const queryCoordinates = axes
            .map(axis => `let query${axis.name.toUpperCase()}Coordinate = getCoordinate(query${axis.name.toUpperCase()}, ${getFloatLiteral(axis.minimum)}, ${getFloatLiteral(axis.maximum)}, ${axis.size});`)
            .join('\n  ');
        const queryCell = dimension === 2
  ...
  let selected = ${valid} && cellIndex == ${queryCell};`;
    ...
    if (query.kind === 'bounds') {
        const values = axes
            .map((axis, axisIndex) => `let queryMin${axis.name.toUpperCase()} = queryValues[QUERY_OFFSET + ${axisIndex}u];
  let queryMax${axis.name.toUpperCase()} = queryValues[QUERY_OFFSET + ${axisIndex + dimension}u];`)
            .join('\n  ');
        const valid = axes
            .map(axis => `finite(queryMin${axis.name.toUpperCase()}) && finite(queryMax${axis.name.toUpperCase()}) && queryMin${axis.name.toUpperCase()} <= queryMax${axis.name.toUpperCase()}`)
            .join(' && ');
        const selected = axes
            .map(axis => `cellMax${axis.name.toUpperCase()} >= queryMin${axis.name.toUpperCase()} && cellMin${axis.name.toUpperCase()} <= queryMax${axis.name.toUpperCase()}`)
            .join(' && ');
  ...
  let selected = ${valid} && ${selected};`;
    ...
    const closestPoints = axes
        .map(axis => `let closest${axis.name.toUpperCase()} = clamp(query${axis.name.toUpperCase()}, cellMin${axis.name.toUpperCase()}, cellMax${axis.name.toUpperCase()});`)
        .join('\n  ');
    ...
    const squaredDistance = axes
        .map(axis => `(query${axis.name.toUpperCase()} / scale - closest${axis.name.toUpperCase()} / scale) * (query${axis.name.toUpperCase()} / scale - closest${axis.name.toUpperCase()} / scale)`)
        .join(' + ');
    return `${cellDeclarations}
  ${values}
...
function makeNestedMaximum(values) {
    return values.slice(1).reduce((maximum, value) => `max(${maximum}, ${value})`, values[0]);
}
function validateDisjointQueryViews(id, index, query, outputs) {
    const inputs = [
    ...
    for (let outputIndex = 0; outputIndex < outputs.length; outputIndex++) {
        ...
        for (let previousIndex = 0; previousIndex < outputIndex; previousIndex++) {
            if (doGraphDataViewsOverlap(output, outputs[previousIndex])) {
                throw new Error(`${id} ${outputNames[outputIndex]} and ${outputNames[previousIndex]} must not overlap`);
            }
        }
    }
}
function validateIndexView(id, index, dimension) {
    ...
    if (!index.bounds.every(Number.isFinite) ||
        Array.from({ length: dimension }, (_, axis) => axis).some(axis => index.bounds[axis] > index.bounds[axis + dimension])) {
...
//# sourceMappingURL=gpu-grid-index-query.js.map