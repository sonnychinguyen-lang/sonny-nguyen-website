...
import { createTransientView, getViewBinding, getViewElementOffset, validateMatchingVectorTopology, validatePackedView, validatePackedUint32View } from "./graph-data-view-utils.js";
...
export class GPUGroupAggregation {
    ...
    addToGraph(graph) {
        ...
        if (keyChunks.some(chunk => chunk.buffer.graph !== graph) ||
        ...
        if (this.operation === 'count') {
            ...
            for (let chunkIndex = 0; chunkIndex < keyChunks.length; chunkIndex++) {
                ...
                addGroupCountPass(graph, {
                    id: this.keys instanceof GraphVectorView
                        ? `${this.id}-chunk-${chunkIndex}-${accumulationPath}`
                        : `${this.id}-${accumulationPath}`,
                    keys,
                    output,
                    mask: maskChunks?.[chunkIndex],
                    dispatchLayout: getGPUGroupAggregationDispatchLayout(keys.length, graph.device.limits.maxComputeWorkgroupsPerDimension)
                });
            }
            return;
        }
...
function validateScalarInputView(view, format, name) {
...
function getValueChunks(input) {
    return input instanceof GraphVectorView ? input.data : [input];
}
/** Validates atomic/vector kind, row count, and ordered vector chunk lengths. */
function validateMatchingInputs(keys, paired, label) {
...
function addClearGroupsPass(graph, id, output) {
    const dispatchLayout = getGPUGroupAggregationDispatchLayout(output.length, graph.device.limits.maxComputeWorkgroupsPerDimension);
    const source = /* wgsl */ `
...
function addGroupCountPass(graph, props) {
    ...
    const localAccumulation = useSubgroups
        ? getSubgroupCoalescedAtomicAddWGSL('accepted', 'groupIndex', 'localCounts', props.output.length)
...
function addInitializeGroupStatisticsPass(graph, id, output, operation, counts) {
    const dispatchLayout = getGPUGroupAggregationDispatchLayout(output.length, graph.device.limits.maxComputeWorkgroupsPerDimension);
  ...
  if (index < GROUP_COUNT) {
    ...
    addComputationPass(graph, {
        ...
        resources: [
            ...
            ...(counts ? [{ buffer: counts, usage: 'storage-write' }] : [])
        ],
...
function addGroupStatisticPass(graph, props) {
    const useSubgroups = props.output.length <= MAXIMUM_SUBGROUP_COALESCED_GROUP_COUNT &&
        getGPUShaderSubgroupStrategy(graph.device) === 'subgroups';
    const maskBinding = props.mask
...
${accumulation}
    ...
    const resources = [
        ...
        { buffer: props.output, usage: 'storage-read-write' },
        ...(props.counts
            ? [{ buffer: props.counts, usage: 'storage-read-write' }]
            : [])
    ];
    addComputationPass(graph, {
...
function addFinalizeGroupStatisticsPass(graph, id, output, operation, counts) {
    const dispatchLayout = getGPUGroupAggregationDispatchLayout(output.length, graph.device.limits.maxComputeWorkgroupsPerDimension);
    const countBinding = counts
        ? '@group(0) @binding(1) var<storage, read> outputCounts: array<u32>;'
        : '';
    const finalizeStatement = operation === 'mean'
        ? `let count = outputCounts[${getViewElementOffset(counts)}u + index];
    if (count == 0u) {
    ...
    const decodeFunction = operation === 'mean'
        ? ''
  ...
  if (index < GROUP_COUNT) {
    ...
    addComputationPass(graph, {
        ...
        resources: [
            ...
            ...(counts ? [{ buffer: counts, usage: 'storage-read' }] : [])
        ],
        bindings: { outputValues: output, ...(counts ? { outputCounts: counts } : {}) },
        dispatchSize: dispatchLayout
    });
...
function getFloatAggregationFunction(operation) {
...
function getFloatAggregationCall(operation, groupIndex, valueExpression = 'value') {
    if (operation === 'min' || operation === 'max') {
...
function getSubgroupStatisticAggregationWGSL(operation, groupCount, counts) {
    ...
    const collective = operation === 'min'
...
export function getGPUGroupAggregationDispatchLayout(elementCount, maxComputeWorkgroupsPerDimension) {
    return getBoundedDispatchLayout('GPUGroupAggregation', elementCount, GROUP_AGGREGATION_WORKGROUP_SIZE, maxComputeWorkgroupsPerDimension);
}
            ...
            return {
                encode: ({ computePass, getBuffer }) => {
                    ...
                    if (props.dispatchSize) {
                        computation.dispatch(computePass, props.dispatchSize.x, props.dispatchSize.y, props.dispatchSize.z);
                    }
                    else {
...
//# sourceMappingURL=gpu-group-aggregation.js.map