...
import { createTransientView, doGraphDataViewsOverlap, getViewBinding, getViewElementOffset, validatePackedUint32View } from "./graph-data-view-utils.js";
...
export class GPUHashIndex {
    ...
    constructor(props) {
        ...
        if (!Number.isSafeInteger(this.firstValue) ||
            this.firstValue < 0 ||
        ...
        validateDisjointViews(this.id, [this.keys, ...(this.values ? [this.values] : [])], [this.tableKeys, this.tableValues, this.statistics]);
    ...
    addToGraph(graph) {
        ...
        const sourceRows = createTransientView(graph, `${this.id}-source-rows`, 'uint32', this.tableKeys.length);
        const batch = {
            id: this.id,
            keys: this.keys,
        ...
        if (this.keys.length > 0)
            addBuildPass(graph, this, sourceRows, batch);
...
export function addGPUHashIndexBuildBatchesToGraph(graph, index, batches) {
    const sourceRows = createTransientView(graph, `${index.id}-source-rows`, 'uint32', index.tableKeys.length);
...
export class GPUHashIndexQuery {
    ...
    constructor(props) {
        ...
        validateDisjointViews(this.id, [this.index.tableKeys, this.index.tableValues, this.keys], [this.values, this.found, this.probes, this.statistics]);
    }
...
function addBuildInitializePass(graph, index, sourceRows) {
    const layout = getDispatchLayout(Math.max(index.tableKeys.length, GPU_HASH_INDEX_STATISTICS_LENGTH), graph.device.limits.maxComputeWorkgroupsPerDimension);
    const source = /* wgsl */ `
...
function addBuildPass(graph, index, sourceRows, batch) {
    const layout = getDispatchLayout(batch.keys.length, graph.device.limits.maxComputeWorkgroupsPerDimension);
  ...
  if (inserted) {
    ...
    addComputationPass(graph, {
        ...
        resources: [
            ...
            ...(batch.validity
                ? [{ buffer: batch.validity, usage: 'storage-read' }]
                : []),
...
function addBuildFinalizePass(graph, index, sourceRows, batch) {
    const layout = getDispatchLayout(index.tableKeys.length, graph.device.limits.maxComputeWorkgroupsPerDimension);
  ...
  tableValues[TABLE_VALUES_OFFSET + slot] = ${valueExpression};
    ...
    addComputationPass(graph, {
        ...
        resources: [
            ...
            { buffer: index.tableValues, usage: 'storage-write' },
            ...(hasExplicitValues
                ? [{ buffer: batch.values, usage: 'storage-read' }]
                : [])
        ],
        bindings: {
            tableKeys: index.tableKeys,
...
function addQueryInitializePass(graph, query) {
    const source = /* wgsl */ `
...
function addQueryPass(graph, query) {
    const layout = getDispatchLayout(query.keys.length, graph.device.limits.maxComputeWorkgroupsPerDimension);
    const source = /* wgsl */ `
...
function validateProbeCount(id, probes, capacity, rows) {
...
function validateDisjointViews(id, inputs, outputs) {
...
function getDispatchLayout(elementCount, maximumDimension) {
...
//# sourceMappingURL=gpu-hash-index.js.map