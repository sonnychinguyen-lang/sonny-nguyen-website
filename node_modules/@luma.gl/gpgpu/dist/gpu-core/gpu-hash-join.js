...
import { GPUHashIndexQuery } from "./gpu-hash-index.js";
...
export class GPUHashJoin {
    ...
    constructor(props) {
        ...
        for (const [view, name] of [
            ...
            ...(this.index.statistics ? [[this.index.statistics, 'index.statistics']] : []),
            [this.keys, 'keys'],
            ...(this.leftRows ? [[this.leftRows, 'leftRows']] : []),
            [this.outputLeftRows, 'outputLeftRows'],
            [this.outputRightRows, 'outputRightRows'],
            [this.count, 'count'],
            [this.overflow, 'overflow'],
            [this.statistics, 'statistics'],
            ...(this.found ? [[this.found, 'found']] : []),
            ...(this.probes ? [[this.probes, 'probes']] : [])
        ]) {
        ...
        if (!Number.isSafeInteger(this.maxProbeCount) ||
        ...
        if (!Number.isSafeInteger(this.firstLeftRow) ||
            this.firstLeftRow < 0 ||
        ...
        this.stats = Object.freeze({
            ...
            outputByteLength: this.outputLeftRows.length * 8 +
    ...
    addToGraph(graph) {
        ...
        const matchedRightRows = createTransientView(graph, `${this.id}-matched-right-rows`, 'uint32', this.keys.length);
        const found = this.found ?? createTransientView(graph, `${this.id}-found`, 'uint32', this.keys.length);
        const probes = this.probes ?? createTransientView(graph, `${this.id}-probes`, 'uint32', this.keys.length);
...
function addEmptyJoinPass(graph, join) {
    const source = /* wgsl */ `
...
function addJoinScatterPass(graph, join, matchedRightRows, found, offsets) {
    if (join.outputLeftRows.length === 0) {
        addJoinCountOnlyPass(graph, join, found, offsets);
        return;
    }
    const layout = getDispatchLayout(join.keys.length, graph.device.limits.maxComputeWorkgroupsPerDimension);
  ...
  if (inputIndex == ELEMENT_COUNT - 1u) {
    ...
    addComputationPass(graph, {
        ...
        resources: [
            ...
            ...(join.leftRows
                ? [{ buffer: join.leftRows, usage: 'storage-read' }]
                : []),
...
function addJoinCountOnlyPass(graph, join, found, offsets) {
    const source = /* wgsl */ `
...
function addSourceOverflowPass(graph, id, indexStatistics, overflow) {
    const source = /* wgsl */ `
...
//# sourceMappingURL=gpu-hash-join.js.map