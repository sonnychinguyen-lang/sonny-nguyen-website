...
import { createTransientView, getViewBinding, getViewElementOffset, validateMatchingVectorTopology, validatePackedUint32View, validatePackedView } from "./graph-data-view-utils.js";
import { GPUReduction } from "./gpu-reduction.js";
import { getGPUShaderSubgroupStrategy, getSubgroupBallotHelpersWGSL, getSubgroupCoalescedAtomicAddWGSL } from "./gpu-subgroup-utils.js";
...
const SCALAR_FORMATS = ['uint32', 'sint32', 'float32'];
/**
...
export class GPUHistogram {
    ...
    constructor(props) {
        ...
        for (const [chunkIndex, input] of getHistogramInputs(this.input).entries()) {
            ...
            validateScalarInputView(input, `${this.id}${name}`);
        ...
        if (this.edges !== undefined) {
            ...
            getHistogramInputs(this.input).some(input => input.byteStride !== input.rowByteLength)) {
            throw new Error(`${this.id} automatic domains require packed input; provide an explicit domain`);
        }
    ...
    addToGraph(graph) {
        ...
        if (inputs.some(input => input.buffer.graph !== graph) ||
        ...
        if (edges !== undefined) {
            ...
            inputs.forEach((input, chunkIndex) => {
                ...
                addIrregularHistogramPass(graph, {
                    id: this.input instanceof GraphVectorView
        ...
        if (domain === 'auto') {
            const inferredDomain = createTransientView(graph, `${this.id}-auto-domain`, this.input.format, 2);
            new GPUReduction({
                id: `${this.id}-extent`,
                input: this.input,
...
function getHistogramInputs(input) {
...
function getHistogramMasks(mask) {
    return mask instanceof GraphVectorView ? mask.data : [mask];
}
function validateScalarInputView(view, name) {
    if (!SCALAR_FORMATS.includes(view.format) ||
        ...
        throw new Error(`${name} must be a uint32-aligned 32-bit scalar GPU data view`);
    }
}
function getScalarStride(view) {
    return view.byteStride / UINT32_BYTE_LENGTH;
}
/** Clears every output bin before accumulation for the current graph encoding. */
function addClearHistogramPass(graph, id, output) {
...
function addValidateHistogramEdgesPass(graph, id, edges, validity) {
    const shaderType = getShaderType(edges.format);
    const finiteCondition = edges.format === 'float32'
...
function addIrregularHistogramPass(graph, props) {
    ...
    const literalEdges = props.edges;
...
fn getEdge(index: u32) -> ${shaderType} { return EDGES[index]; }`;
    const validityCondition = props.edgeValidity
        ...
        : 'true';
    const finiteCondition = props.input.format === 'float32' ? 'value == value && abs(value) <= 3.402823466e+38' : 'true';
...
const INPUT_STRIDE: u32 = ${getScalarStride(props.input)}u;
${gpuEdges ? `const EDGE_OFFSET: u32 = ${getViewElementOffset(props.edges)}u;` : ''}
${props.mask
        ? `const MASK_OFFSET: u32 = ${getViewElementOffset(props.mask)}u;
const MASK_STRIDE: u32 = ${getScalarStride(props.mask)}u;`
        : ''}
...
@compute @workgroup_size(${HISTOGRAM_WORKGROUP_SIZE}) fn main(
  ...
  var accepted = false;
  var binIndex = 0u;
  if (index < ELEMENT_COUNT && ${validityCondition}${props.mask ? ' && selectionMask[MASK_OFFSET + index * MASK_STRIDE] != 0u' : ''}) {
    let value = inputValues[INPUT_OFFSET + index * INPUT_STRIDE];
  ...
  ${accumulation}
    ...
    const resources = [
        ...
        ...(gpuEdges
            ? [{ buffer: props.edges, usage: 'storage-read' }]
            : []),
        ...(props.edgeValidity
            ? [{ buffer: props.edgeValidity, usage: 'storage-read' }]
            : []),
        ...(props.mask ? [{ buffer: props.mask, usage: 'storage-read' }] : []),
    ...
    addComputationPass(graph, {
        ...
        bindings: {
            ...
            ...(gpuEdges ? { edgeValues: props.edges } : {}),
...
function addHistogramPass(graph, props) {
    const dispatchLayout = getHistogramDispatchLayout(graph, props.input.length);
    const local = props.output.length <= MAXIMUM_LOCAL_BIN_COUNT;
    const useSubgroups = local &&
        props.output.length <= MAXIMUM_SUBGROUP_COALESCED_BIN_COUNT &&
    ...
    const literalDomain = props.domain;
  ...
  let maximum: ${shaderType} = ${getLiteral(literalDomain[1], props.input.format)};`;
    ...
    const integerBinningFunction = props.input.format === 'float32'
        ? ''
  ...
  return quotient;
    ...
    const binCalculation = props.input.format === 'float32'
...
const INPUT_OFFSET: u32 = ${getViewElementOffset(props.input)}u;
const INPUT_STRIDE: u32 = ${getScalarStride(props.input)}u;
${gpuDomain ? `const DOMAIN_OFFSET: u32 = ${getViewElementOffset(props.domain)}u;` : ''}
${props.mask
        ? `const MASK_OFFSET: u32 = ${getViewElementOffset(props.mask)}u;
const MASK_STRIDE: u32 = ${getScalarStride(props.mask)}u;`
        : ''}
...
@compute @workgroup_size(${HISTOGRAM_WORKGROUP_SIZE}) fn main(
  ...
  var accepted = false;
  var binIndex = 0u;
  if (index < ELEMENT_COUNT && maximum >= minimum${props.mask ? ' && selectionMask[MASK_OFFSET + index * MASK_STRIDE] != 0u' : ''}) {
    let value = inputValues[INPUT_OFFSET + index * INPUT_STRIDE];
    if (${finiteCondition} && value >= minimum && value <= maximum) {
      if (maximum == minimum) {
  ...
  ${accumulation}
    ...
    const resources = [
        { buffer: props.input, usage: 'storage-read' },
        ...(gpuDomain
            ? [{ buffer: props.domain, usage: 'storage-read' }]
            : []),
        ...(props.mask ? [{ buffer: props.mask, usage: 'storage-read' }] : []),
        { buffer: props.output, usage: 'storage-read-write' }
    ];
    addComputationPass(graph, {
        ...
        bindings: {
            inputValues: props.input,
            ...(gpuDomain ? { domainValues: props.domain } : {}),
            ...(props.mask ? { selectionMask: props.mask } : {}),
            outputCounts: props.output
        },
        dispatchLayout
...
function getHistogramDispatchLayout(graph, elementCount) {
    return getBoundedDispatchLayout('GPUHistogram', elementCount, HISTOGRAM_WORKGROUP_SIZE, graph.device.limits.maxComputeWorkgroupsPerDimension);
}
/** Validates literal or GPU-resident irregular histogram boundaries. */
function validateHistogramEdges(edges, format, output, id) {
    ...
    const invalidValue = format === 'float32'
...
function validateLiteralDomain(domain, format, id) {
...
function getLiteral(value, format) {
    if (format === 'uint32')
        return `${value}u`;
    if (format === 'sint32')
        return `${value}`;
...
//# sourceMappingURL=gpu-histogram.js.map