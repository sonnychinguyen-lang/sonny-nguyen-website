 ...
 */
export class GPUIndexPickingTarget {
    ...
    graph;
    readbackHandle = null;
    readbackPassAdded = false;
    regionPassAdded = false;
    /**
...
${useSubgroups
  ...
  if (outputIndex < RESULT_CAPACITY) {
    ...
    atomicStore(&result[RESULT_OFFSET + 1u], 1u);
                ...
                return {
                    encode: ({ computePass, getBuffer, getTextureView }) => {
                        ...
                        computation.dispatch(computePass, Math.ceil(this.width / 8), Math.ceil(this.height / 8));
                    },
                    destroy: () => computation.destroy()
                };
            }
    ...
    createReadbackHandle(readbackBuffer) {
 ...
 */
export function decodeGPUIndexPickInfo(data) {
...
export function decodeGPUIndexPickRegion(data) {
    const byteOffset = ArrayBuffer.isView(data) ? data.byteOffset : 0;
    const byteLength = ArrayBuffer.isView(data) ? data.byteLength : data.byteLength;
    const buffer = ArrayBuffer.isView(data) ? data.buffer : data;
    ...
    const storedCount = Math.min(count, Math.floor((byteLength / Uint32Array.BYTES_PER_ELEMENT - 2) / 2));
    const picks = [];
...
function validatePickingTargetSize(width, height) {
...
function validatePickingPixel(x, y, width, height) {
    if (!Number.isSafeInteger(x) ||
        ...
        x >= width ||
        y >= height) {
        throw new Error(`GPUIndexPickingTarget pixel [${x}, ${y}] is outside ${width}x${height} target`);
    }
}
//# sourceMappingURL=gpu-index-picking-target.js.map