...
import { GPUScan } from "./gpu-scan.js";
...
export class GPUIndexedRangeCompaction {
    ...
    constructor(props) {
        ...
        if (!Number.isSafeInteger(this.maximumRangeLength) ||
    ...
    addToGraph(graph) {
        ...
        const localOffsets = createTransientView(graph, `${this.id}-local-offsets`, 'uint32', this.flags.length);
        const rangeCounts = createTransientView(graph, `${this.id}-range-counts`, 'uint32', this.rangeCount);
        const rangeOffsets = createTransientView(graph, `${this.id}-range-offsets`, 'uint32', this.rangeCount);
...
export class GPUPartitionedIndexedRangeCompaction {
    ...
    constructor(props) {
        ...
        validatePartitionRangeEnds(this.id, this.partitionRangeEnds, this.flags.data.length, this.rangeCount);
    }
    ...
    addToGraph(graph) {
        ...
        const rangeCounts = createTransientView(graph, `${this.id}-range-counts`, 'uint32', this.rangeCount);
        const rangeOffsets = createTransientView(graph, `${this.id}-range-offsets`, 'uint32', this.rangeCount);
        const partitionCounts = createTransientView(graph, `${this.id}-partition-counts`, 'uint32', this.partitionRangeEnds.length);
        addClearRangeCountsPass(graph, this.id, rangeCounts);
        let sourceStart = 0;
        let rangeStart = 0;
        for (let partitionIndex = 0; partitionIndex < this.flags.data.length; partitionIndex++) {
            ...
            const partitionRangeCounts = graph.createDataView(rangeCounts.buffer, {
                format: 'uint32',
                length: partitionRangeCount,
                byteOffset: rangeStart * Uint32Array.BYTES_PER_ELEMENT
            });
            const partitionRangeOffsets = graph.createDataView(rangeOffsets.buffer, {
                format: 'uint32',
                length: partitionRangeCount,
                byteOffset: rangeStart * Uint32Array.BYTES_PER_ELEMENT
            });
...
function validateBitsetVectorTopology(id, flags, output) {
...
function validatePartitionedVector(id, name, vector) {
...
function validateRangeCompactionShape(compaction) {
    if (!Number.isSafeInteger(compaction.rangeCount) ||
    ...
    if (!Number.isSafeInteger(compaction.maximumRangeLength) ||
...
function validatePartitionRangeEnds(id, partitionRangeEnds, partitionCount, rangeCount) {
...
function addPartitionRangeCountPass(graph, compaction, props) {
...
function addPartitionScatterPass(graph, compaction, props) {
    const { rangeLayout } = compaction;
    const useSubgroups = getGPUShaderSubgroupStrategy(graph.device, { requiresSubgroupId: true }) === 'subgroups';
    const source = /* wgsl */ `
${useSubgroups ? 'enable subgroups;\nrequires subgroup_id;' : ''}
...
function getPartitionSelectionScanShader(encoding, useSubgroups) {
...
function getFlagSelectionExpression(encoding, sourceIndex) {
    return encoding === 'bitset'
        ? `(flags[FLAGS_OFFSET + (${sourceIndex} >> 5u)] & (1u << (${sourceIndex} & 31u))) != 0u`
        : `flags[FLAGS_OFFSET + ${sourceIndex}] != 0u`;
}
function addPartitionCountPass(graph, compaction, rangeCounts, rangeOffsets, partitionCounts) {
...
function validateRangeLayout(id, layout) {
...
function addClearRangeCountsPass(graph, id, rangeCounts) {
...
function addLocalScanPass(graph, compaction, localOffsets, rangeCounts) {
    const { rangeLayout } = compaction;
...
function getRangeSelectionScanShader(useSubgroups) {
    if (useSubgroups) {
        return /* wgsl */ `
  let localIndex = subgroupId * subgroupSize + subgroupInvocationId;
  var selected = 0u;
...
function addScatterPass(graph, compaction, localOffsets, rangeOffsets) {
    const { rangeLayout } = compaction;
...
function addCountPass(graph, id, rangeCounts, rangeOffsets, count) {
...
function addDirectPass(graph, props) {
...
function addIndirectPass(graph, props) {
    graph.addComputePass({
        id: props.id,
...
function makeComputation(device, id, source, views) {
...
function resolveBindings(views, getBuffer) {
    const bindings = {};
...
//# sourceMappingURL=gpu-indexed-range-compaction.js.map