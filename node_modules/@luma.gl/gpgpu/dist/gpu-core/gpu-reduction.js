...
export function getGPUReductionStrategy(device) {
        ...
        if (nonEmptyInputs.length === 1) {
            ...
            nonEmptyInputs.forEach(({ input, mask }, partialIndex) => {
                addReductionLevels(graph, {
                    ...
                    firstLevel: true,
                    needsValidity,
                    finalValues: createPackedSubview(graph, partialValues, partialIndex * valuesPerRow, valuesPerRow),
...
function addReductionLevels(graph, props) {
    ...
    while (currentLength > 1 || levelIndex === 0) {
        ...
        const nextValues = isLastLevel && props.finalValues
            ? props.finalValues
            : createTransientView(graph, `${props.id}-level-${levelIndex}-values`, props.format, nextLength * props.valuesPerRow);
        const nextValidity = props.needsValidity
            ? isLastLevel && props.finalValidity
                ? props.finalValidity
                : createTransientView(graph, `${props.id}-level-${levelIndex}-validity`, 'uint32', nextLength)
            : undefined;
...
function createPackedSubview(graph, view, elementOffset, length) {
...
function addReductionLevelPass(graph, props) {
    ...
    const reductionSource = strategy === 'subgroups'
...
@compute @workgroup_size(${REDUCTION_WORKGROUP_SIZE}) fn main(
  ...
  if (lane == 0u) {
    ...
    const resources = [
        ...
        ...(props.inputValidity
            ? [{ buffer: props.inputValidity, usage: 'storage-read' }]
            : []),
        { buffer: props.outputValues, usage: 'storage-write' },
        ...(props.outputValidity
            ? [{ buffer: props.outputValidity, usage: 'storage-write' }]
            : [])
    ];
...
//# sourceMappingURL=gpu-reduction.js.map