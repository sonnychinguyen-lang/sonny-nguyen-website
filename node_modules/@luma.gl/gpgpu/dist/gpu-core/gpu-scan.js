...
import { GraphVectorView } from "./gpu-command-graph.js";
import { getBoundedDispatchLayout, getBoundedInvocationIndexSource } from "./gpu-dispatch-utils.js";
import { createTransientView, getViewBinding, getViewElementOffset, validateMatchingVectorTopology, validatePackedUint32View } from "./graph-data-view-utils.js";
import { getGPUShaderSubgroupStrategy } from "./gpu-subgroup-utils.js";
...
export function getGPUScanStrategy(device, segmented = false) {
...
export function addGPUScanToGraphWithDispatchLimit(scan, graph, maxComputeWorkgroupsPerDimension) {
...
function addChunkedScan(graph, props, maxComputeWorkgroupsPerDimension) {
    ...
    const chunkSegmentPrefixes = props.segmentFlags
        ? nonEmptyChunks.map(chunk => createTransientView(graph, `${props.id}-chunk-${chunk.chunkIndex}-segment-prefixes`, 'uint32', chunk.input.length))
        : undefined;
        ...
        const segmentPrefixes = levelSegmentFlags
            ? levelIndex === 0 && props.outputSegmentPrefixes
                ...
                : blockCount > 1 || levelIndex > 0
                    ? createTransientView(graph, `${props.id}-level-${levelIndex}-segment-prefixes`, 'uint32', levelLength)
                    : undefined
            : undefined;
...
function addBlockScanPass(graph, props) {
    ...
    const segmentedOutput = props.segmentFlags && props.mode === 'exclusive'
...
@compute @workgroup_size(${SCAN_WORKGROUP_SIZE}) fn main(
  ...
  if (index < ELEMENT_COUNT) {
    ...
    const source = strategy === 'subgroups'
    ...
    graph.addComputePass({
        ...
        resources: [
            ...
            ...(props.segmentPrefixes
                ? [{ buffer: props.segmentPrefixes, usage: 'storage-write' }]
                : []),
            ...
            const computation = new Computation(device, {
                ...
                shaderLayout: {
                    bindings: [
                        ...
                        ...(sumOutput
                            ? [{ name: 'sumValues', type: 'storage', group: 0, location: 2 }]
                            : []),
                        ...(props.segmentFlags
                            ? [{ name: 'segmentFlags', type: 'storage', group: 0, location: 3 }]
                            : []),
                        ...(props.segmentPrefixes
                            ? [{ name: 'segmentPrefixes', type: 'storage', group: 0, location: 4 }]
                            : []),
                        ...(segmentFlagOutput
                            ? [
                                {
    ...
    return graph.autotuner.selectKernel({
        ...
        candidates: [
            { id: preferredStrategy },
            {
                ...
                supported: alternateStrategy === 'portable' ||
...
function addOffsetPass(graph, props) {
    const offsetIndex = props.offsetIndex === undefined ? `index / ${SCAN_WORKGROUP_SIZE}u` : `${props.offsetIndex}u`;
    const source = /* wgsl */ `
const ELEMENT_COUNT: u32 = ${props.length}u;
const OUTPUT_OFFSET: u32 = ${getViewElementOffset(props.output)}u;
    ...
    graph.addComputePass({
        ...
        resources: [
            ...
            ...(props.offsetSegmentPrefixes
                ? [{ buffer: props.offsetSegmentPrefixes, usage: 'storage-read' }]
                : [])
        ],
        compile: ({ device }) => {
            const computation = new Computation(device, {
                ...
                shaderLayout: {
                    bindings: [
                        ...
                        ...(props.segmentPrefixes
                            ? [{ name: 'segmentPrefixes', type: 'storage', group: 0, location: 2 }]
                            : []),
                        ...(props.offsetSegmentPrefixes
                            ? [
                                {
...
export function getGPUScanDispatchLayout(elementCount, maxComputeWorkgroupsPerDimension) {
...
export function getGPUScanInvocationIndexSource(layout) {
    return getBoundedInvocationIndexSource(layout, SCAN_WORKGROUP_SIZE);
}
//# sourceMappingURL=gpu-scan.js.map