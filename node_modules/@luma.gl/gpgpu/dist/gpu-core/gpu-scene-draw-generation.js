...
import { GPU_SCENE_ACTIVE_FLAG, GPU_SCENE_INVALID_REFERENCE } from "./gpu-scene.js";
...
export class GPUSceneDrawGeneration {
    ...
    constructor(props) {
        ...
        for (const [view, name] of [
            ...
            if (view.length < 1)
                throw new Error(`${this.id} ${name} requires one uint32 row`);
        }
        ...
        this.stats = Object.freeze({
            ...
            outputByteLength: this.commands.capacity * this.commands.recordByteLength + 3 * UINT32_BYTE_LENGTH
        });
    }
    ...
    addToGraph(graph) {
        ...
        const owners = createTransientView(graph, `${this.id}-owners`, 'uint32', this.commands.capacity);
        const eligibility = createTransientView(graph, `${this.id}-eligibility`, 'uint32', this.scene.flags.length);
...
function addEligibilityPass(graph, generation, sceneRecords, eligibility) {
    const dispatch = getDispatchLayout(generation.scene.flags.length, graph.device.limits.maxComputeWorkgroupsPerDimension);
  ...
  eligibility[ELIGIBILITY_OFFSET + index] = eligible;
        ...
        resources: [
            ...
            ...(generation.visibility
                ? [{ buffer: generation.visibility, usage: 'storage-read' }]
                : []),
...
function addInitializePass(graph, generation, owners) {
    const recordWords = generation.commands.recordByteLength / UINT32_BYTE_LENGTH;
    const firstInstanceWord = recordWords - 1;
    const dispatch = getDispatchLayout(Math.max(generation.commands.capacity, 1), graph.device.limits.maxComputeWorkgroupsPerDimension);
    addComputationPass(graph, {
...
function addClaimPass(graph, generation, eligibility, owners) {
...
function addPublishPass(graph, generation, eligibility, owners) {
    const useSubgroups = getGPUShaderSubgroupStrategy(graph.device) === 'subgroups';
    const recordWords = generation.commands.recordByteLength / UINT32_BYTE_LENGTH;
    const firstInstanceWord = recordWords - 1;
    const dispatch = getDispatchLayout(generation.scene.flags.length, graph.device.limits.maxComputeWorkgroupsPerDimension);
...
function validateSceneSource(id, scene, visibility) {
    if (!Number.isSafeInteger(scene.recordCount) ||
        scene.recordCount < 0 ||
    ...
    if (scene.flags.buffer !== scene.commandSlots.buffer ||
...
function validateSceneField(view, name) {
...
function validateCommandView(id, commands) {
    const recordWords = commands.type === 'draw-indexed' ? 5 : 4;
    if (!Number.isSafeInteger(commands.capacity) ||
        commands.capacity < 1 ||
...
function getDispatchLayout(elementCount, maximumDimension) {
    ...
    if (z > maximum)
        throw new Error('GPU scene draw generation exceeds the device dispatch limit');
    return { x, y, z };
}
function makeDispatchConstants(dispatch) {
...
//# sourceMappingURL=gpu-scene-draw-generation.js.map