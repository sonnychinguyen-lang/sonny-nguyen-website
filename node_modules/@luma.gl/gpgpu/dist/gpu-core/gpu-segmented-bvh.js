...
import { getBoundedDispatchLayout } from "./gpu-dispatch-utils.js";
import { getViewBinding, getViewElementOffset, validatePackedUint32View, validatePackedView } from "./graph-data-view-utils.js";
...
export class GPUSegmentedBVH {
    ...
    constructor(props) {
        ...
        if (this.nodeMinima.format !== this.minima.format ||
        ...
        for (const [outputIndex, output] of outputs.entries()) {
            if (inputs.some(input => input.buffer === output.buffer) ||
        ...
        this.segments = props.segments.map((segment, segmentIndex) => validateSegment(this, segment, segmentIndex));
    ...
    addToGraph(graph) {
        addGPUSegmentedBVHToGraphWithDispatchLimit(this, graph, graph.device.limits.maxComputeWorkgroupsPerDimension);
    }
}
/** Adds packed hierarchy work while propagating a bounded 3D dispatch limit. @internal */
export function addGPUSegmentedBVHToGraphWithDispatchLimit(hierarchy, graph, maxComputeWorkgroupsPerDimension) {
    for (const view of [
        hierarchy.minima,
        hierarchy.maxima,
    ...
    const plans = Array.from(groups, ([leafCapacity, segments]) => {
        ...
        return {
            leafCapacity,
            segments,
            dispatchLayout: getBoundedDispatchLayout(`${hierarchy.id} ${leafCapacity}-leaf hierarchies`, segments.length * dispatchPlanningWidth, dispatchPlanningWidth, maxComputeWorkgroupsPerDimension)
        };
    });
...
function validateSegment(hierarchy, segment, segmentIndex) {
    const name = `${hierarchy.id} segment ${segmentIndex}`;
    if (!Number.isSafeInteger(segment.leafCapacity) ||
    ...
    const limits = hierarchy.minima.buffer.graph.device.limits;
    if (segment.leafCapacity > limits.maxComputeInvocationsPerWorkgroup ||
    ...
    validateRange(name, 'sourceOffset', segment.sourceOffset, segment.sourceCount, hierarchy.minima.length);
    validateRange(name, 'nodeOffset', segment.nodeOffset, segment.leafCapacity * 2 - 1, hierarchy.nodeMinima.length);
    validateRange(name, 'leafOffset', segment.leafOffset, segment.leafCapacity, hierarchy.leafIds.length);
...
function validateRange(name, field, offset, length, capacity) {
...
function validateDisjointRanges(segments, field, getLength) {
...
function groupSegmentsByLeafCapacity(segments) {
    ...
    return new Map(Array.from(groups).sort(([firstCapacity], [secondCapacity]) => firstCapacity - secondCapacity));
}
/** Records all equally sized complete-binary hierarchies in one CORE-compatible graph node. */
function addHierarchyBucketPass(graph, hierarchy, leafCapacity, segments, dispatchLayout) {
    const descriptorSource = segments
        .map(segment => `  BVHSegment(${segment.sourceOffset}u, ${segment.sourceCount}u, ` +
...
//# sourceMappingURL=gpu-segmented-bvh.js.map