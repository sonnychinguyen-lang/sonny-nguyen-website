...
import { getViewBinding, getViewElementOffset, validatePackedUint32View } from "./graph-data-view-utils.js";
...
export class GPUSegmentedSort {
    ...
    constructor(props) {
        ...
        if (this.outputKeys.buffer === this.outputValues.buffer ||
     ...
     */
    addToGraph(graph) {
        addGPUSegmentedSortToGraphWithDispatchLimit(this, graph, graph.device.limits.maxComputeWorkgroupsPerDimension);
    }
}
/** Adds independent stable local domains while propagating a bounded dispatch limit. @internal */
export function addGPUSegmentedSortToGraphWithDispatchLimit(sort, graph, maxComputeWorkgroupsPerDimension) {
    ...
    const plans = Array.from(groups, ([width, segments]) => ({
        width,
        segments,
        dispatchLayout: getBoundedDispatchLayout(`${sort.id} ${width}-wide segments`, segments.length * width, width, maxComputeWorkgroupsPerDimension)
    }));
...
function validateSegment(sort, segment, segmentIndex) {
    const segmentName = `${sort.id} segment ${segmentIndex}`;
    if (!Number.isInteger(segment.length) ||
        segment.length < 0 ||
...
function validateDisjointOutputSegments(segments, offsetName, name) {
...
function groupSegmentsByWidth(segments) {
    ...
    return new Map(Array.from(groups).sort(([firstWidth], [secondWidth]) => firstWidth - secondWidth));
}
/** Records one complete stable sorting network for every equal-width segment workgroup. */
function addSegmentBucketPass(graph, sort, width, segments, dispatchLayout) {
    const descriptorSource = segments
        .map(segment => `  SortSegment(${segment.keysOffset}u, ${segment.valuesOffset}u, ` +
...
function getPortableSegmentedBitonicShader(width) {
    return /* wgsl */ `
...
function getSubgroupSegmentedBitonicShader(width) {
    return /* wgsl */ `
...
//# sourceMappingURL=gpu-segmented-sort.js.map