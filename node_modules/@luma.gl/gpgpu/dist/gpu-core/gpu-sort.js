...
import { addGPUScanToGraphWithDispatchLimit, GPUScan } from "./gpu-scan.js";
import { getGPUShaderSubgroupStrategy } from "./gpu-subgroup-utils.js";
import { createTransientView, getViewBinding, getViewElementOffset, validatePackedUint32View } from "./graph-data-view-utils.js";
...
export class GPUSort {
     ...
     */
    addToGraph(graph) {
        addGPUSortToGraphWithDispatchLimit(this, graph, graph.device.limits.maxComputeWorkgroupsPerDimension);
    }
}
/** Adds one stable sort while propagating an explicit bounded dispatch limit. @internal */
export function addGPUSortToGraphWithDispatchLimit(sort, graph, maxComputeWorkgroupsPerDimension) {
    ...
    const dispatchLayout = getBoundedDispatchLayout('GPUSort', sort.keys.length, RADIX_WORKGROUP_SIZE, maxComputeWorkgroupsPerDimension);
...
function validateSeparateWritableBuffers(sort) {
    if (sort.outputKeys.buffer === sort.outputValues.buffer ||
...
function addCopyPairPass(graph, sort, inputKeys = sort.keys, inputValues = sort.values, identifier = 'copy-pair', dispatchLayout = { x: 1, y: 1, z: 1 }) {
    const source = /* wgsl */ `
...
function addBitonicSort(graph, sort, dispatchLayout, maxComputeWorkgroupsPerDimension) {
    ...
    const paddedDispatchLayout = getBoundedDispatchLayout('GPUSort bitonic', paddedLength, BITONIC_WORKGROUP_SIZE, maxComputeWorkgroupsPerDimension);
    const indicesA = createTransientView(graph, `${sort.id}-bitonic-indices-a`, 'uint32', paddedLength);
    const indicesB = createTransientView(graph, `${sort.id}-bitonic-indices-b`, 'uint32', paddedLength);
    ...
    for (const stage of getBitonicStages(paddedLength)) {
        addBitonicStagePass(graph, sort, currentIndices, nextIndices, paddedLength, stage, paddedDispatchLayout);
...
function addLocalBitonicSortPass(graph, sort, paddedLength) {
    const descending = sort.direction === 'descending';
    const useSubgroups = getGPUShaderSubgroupStrategy(graph.device, { requiresSubgroupId: true }) === 'subgroups';
    const source = /* wgsl */ `
...
function getPortableLocalBitonicShader() {
    return /* wgsl */ `
...
function getSubgroupLocalBitonicShader() {
    return /* wgsl */ `
...
function addBitonicInitializePass(graph, sort, indices, paddedLength, dispatchLayout) {
    const source = /* wgsl */ `
const INVALID_INDEX: u32 = ${INVALID_INDEX}u;
const LOGICAL_LENGTH: u32 = ${sort.keys.length}u;
const PADDED_LENGTH: u32 = ${paddedLength}u;
...
function addBitonicStagePass(graph, sort, indicesIn, indicesOut, paddedLength, stage, dispatchLayout) {
    const descending = sort.direction === 'descending';
    const source = /* wgsl */ `
const INVALID_INDEX: u32 = ${INVALID_INDEX}u;
const LOGICAL_LENGTH: u32 = ${sort.keys.length}u;
...
function addBitonicGatherPass(graph, sort, indices, dispatchLayout) {
    const source = /* wgsl */ `
const LOGICAL_LENGTH: u32 = ${sort.keys.length}u;
const KEYS_OFFSET: u32 = ${getViewElementOffset(sort.keys)}u;
const VALUES_OFFSET: u32 = ${getViewElementOffset(sort.values)}u;
...
function addRadixSort(graph, sort, dispatchLayout, maxComputeWorkgroupsPerDimension) {
    ...
    for (let digitIndex = 0; digitIndex < digitCount; digitIndex++) {
        ...
        const histogram = createTransientView(graph, `${sort.id}-radix-digit-${bitOffset}-histogram`, 'uint32', bucketCount * workgroupCount);
        const offsets = createTransientView(graph, `${sort.id}-radix-digit-${bitOffset}-offsets`, 'uint32', bucketCount * workgroupCount);
        ...
        addRadixHistogramPass(graph, sort, currentKeys, histogram, bitOffset, digitBits, workgroupCount, dispatchLayout);
        ...
        addRadixScatterPass(graph, sort, currentKeys, currentValues, offsets, nextKeys, nextValues, bitOffset, digitBits, workgroupCount, dispatchLayout);
        currentKeys = nextKeys;
        currentValues = nextValues;
    }
}
/** Counts one radix digit per workgroup into a digit-major histogram suitable for global scan. */
function addRadixHistogramPass(graph, sort, keys, histogram, bitOffset, digitBits, workgroupCount, dispatchLayout) {
    const bucketCount = 2 ** digitBits;
    const descending = sort.direction === 'descending';
    const source = /* wgsl */ `
const ELEMENT_COUNT: u32 = ${sort.keys.length}u;
...
function addRadixScatterPass(graph, sort, keys, values, offsets, outputKeys, outputValues, bitOffset, digitBits, workgroupCount, dispatchLayout) {
    const bucketCount = 2 ** digitBits;
    const descending = sort.direction === 'descending';
    const source = /* wgsl */ `
const ELEMENT_COUNT: u32 = ${sort.keys.length}u;
...
function getNextPowerOfTwo(length) {
    let paddedLength = 1;
...
function getBitonicStages(paddedLength) {
    const stages = [];
...
//# sourceMappingURL=gpu-sort.js.map