...
import { GPUCompaction } from "./gpu-compaction.js";
...
export class GPUTextSelection {
    ...
    constructor(props) {
        ...
        const inferredRecordWordLength = this.sourceRecords && this.glyphRows.length > 0
        ...
        if (this.sourceRecords) {
            ...
            if (this.sourceRecords.length < requiredWordCount ||
            ...
            if (this.sourceRecords.byteStride !== Uint32Array.BYTES_PER_ELEMENT ||
    ...
    addToGraph(graph) {
        ...
        const glyphIds = createTransientView(graph, `${this.id}-glyph-ids`, 'uint32', this.glyphRows.length);
        const glyphFlags = createTransientView(graph, `${this.id}-glyph-flags`, 'uint32', this.glyphRows.length);
...
function addGatherGlyphRecordsPass(graph, selection) {
...
function addGlyphVisibilityPass(graph, selection, glyphIds, glyphFlags) {
...
function addClearTextSelectionCount(graph, id, count) {
    graph.addComputePass({
        id: `${id}-clear-count`,
...
@compute @workgroup_size(1) fn main() { count[COUNT_OFFSET] = 0u; }`,
            ...
            return {
                encode: ({ computePass, getBuffer }) => {
                    const bindings = { count: getViewBinding(count, getBuffer) };
...
//# sourceMappingURL=gpu-text-selection.js.map