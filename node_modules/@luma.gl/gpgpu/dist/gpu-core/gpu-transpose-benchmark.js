...
import { GPUCommandGraph } from "./gpu-command-graph.js";
import { getBoundedDispatchLayout, getBoundedInvocationIndexSource } from "./gpu-dispatch-utils.js";
import { getViewBinding, getViewElementOffset } from "./graph-data-view-utils.js";
import { GPUTranspose, makeGPUTransposeStats } from "./gpu-transpose.js";
import { summarizeGPUWorkgroupScanBenchmarkSamples } from "./gpu-workgroup-scan-benchmark.js";
 ...
 */
export async function runGPUTransposeBenchmark(device, props = {}) {
    ...
    validateGPUTransposeBenchmarkProps(device, stats.elementCount, warmupIterations, measuredIterations);
    const inputValues = Float32Array.from({ length: stats.elementCount }, (_, index) => ((index * 2654435761) >>> 8) / 0x1000000);
    ...
    const paths = [];
    const strategies = ['tiled', 'reference'];
    const samples = new Map(strategies.map(strategy => [strategy, { cpu: [], gpu: [] }]));
    ...
    try {
        ...
        for (const path of paths) {
            for (let iteration = 0; iteration < warmupIterations; iteration++) {
                encodeAndSubmitTransposeBenchmark(device, path, `${id}-${path.strategy}-warmup-${iteration}`);
            }
...
function makeGPUTransposeBenchmarkPath(device, benchmarkId, inputBuffer, rows, columns, strategy) {
...
function addGPUTransposeReferencePass(graph, benchmarkId, input, output, rows, columns) {
    const elementCount = rows * columns;
    const dispatchLayout = getBoundedDispatchLayout('GPUTranspose reference', elementCount, REFERENCE_WORKGROUP_SIZE, graph.device.limits.maxComputeWorkgroupsPerDimension);
...
function importGPUTransposeBenchmarkView(graph, id, buffer, length) {
...
function encodeAndSubmitTransposeBenchmark(device, path, id) {
...
async function validateGPUTransposeBenchmarkPath(path, inputValues, rows, columns, benchmarkId) {
    const outputBytes = await path.outputBuffer.readAsync();
    const outputValues = new Float32Array(outputBytes.buffer, outputBytes.byteOffset, inputValues.length);
...
function validateGPUTransposeBenchmarkProps(device, elementCount, warmupIterations, measuredIterations) {
    if (elementCount === 0) {
        throw new Error('GPU transpose benchmark dimensions must be positive');
    }
    if (!Number.isSafeInteger(warmupIterations) ||
        warmupIterations < 1 ||
...
//# sourceMappingURL=gpu-transpose-benchmark.js.map