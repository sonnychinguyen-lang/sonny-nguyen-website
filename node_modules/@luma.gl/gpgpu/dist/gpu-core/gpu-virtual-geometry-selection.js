...
import { GPUVisibilityWorkflow } from "./gpu-visibility-workflow.js";
 ...
 */
export function makeGPUVirtualGeometrySelectionPlan(levelOffsets, nodeCount) {
    ...
    for (let index = 1; index < levelOffsets.length; index++) {
        ...
        if (!Number.isSafeInteger(offset) || offset <= levelOffsets[index - 1]) {
            throw new Error('GPU virtual geometry levelOffsets must be strictly increasing safe integers');
        }
    }
...
export class GPUVirtualGeometrySelection {
    ...
    constructor(props) {
        ...
        this.plan = makeGPUVirtualGeometrySelectionPlan(this.hierarchy.levelOffsets, this.hierarchy.sphereBounds.length);
        ...
        validatePackedView(this.view.pixelProjectionScale, ['float32'], `${this.id} pixelProjectionScale`);
        validatePackedView(this.view.maximumScreenSpaceError, ['float32'], `${this.id} maximumScreenSpaceError`);
        if (this.view.frustumPlanes.length !== GPU_VIRTUAL_GEOMETRY_FRUSTUM_PLANE_COUNT ||
            this.view.cameraPosition.length !== 1 ||
            this.view.pixelProjectionScale.length !== 1 ||
            this.view.maximumScreenSpaceError.length !== 1) {
            throw new Error(`${this.id} view requires six frustum planes and one row for each camera parameter`);
        }
        ...
        if (this.totalCount)
            validatePackedUint32View(this.totalCount, `${this.id} totalCount`);
    ...
    addToGraph(graph) {
        ...
        const activeNodes = this.createOwnedView(graph, `${this.id}-active-nodes`, 'uint32', this.plan.nodeCount);
        const selectedMask = this.createOwnedView(graph, `${this.id}-selected-mask`, 'uint32', this.plan.nodeCount);
        ...
        const compactedClusterIds = this.createOwnedView(graph, `${this.id}-compacted-cluster-ids`, 'uint32', this.plan.nodeCount);
        const selectedTotalCount = this.createOwnedView(graph, `${this.id}-selected-total-count`, 'uint32', 1);
    ...
    createOwnedView(graph, id, format, length) {
...
function addInitializePass(graph, selection, activeNodes, selectedMask) {
    const source = /* wgsl */ `
...
function addPackViewPass(graph, selection, packedView) {
    const source = /* wgsl */ `
...
function addTraversalPass(graph, selection, activeNodes, selectedMask, packedView, levelIndex) {
...
function addFinalizePass(graph, selection, compactedClusterIds, selectedTotalCount) {
  ...
  if (outputIndex == 0u) {
    ...
    addComputationPass(graph, {
        ...
        resources: [
            ...
            ...(selection.totalCount
                ? [{ buffer: selection.totalCount, usage: 'storage-write' }]
                : [])
        ],
        bindings: {
...
//# sourceMappingURL=gpu-virtual-geometry-selection.js.map