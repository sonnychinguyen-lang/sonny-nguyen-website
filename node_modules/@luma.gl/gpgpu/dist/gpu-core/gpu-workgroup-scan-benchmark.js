 ...
 */
export async function runGPUWorkgroupScanBenchmark(device, props = {}) {
    ...
    validateProps(device, workgroupCount, roundCount, dispatchCount, warmupIterations, measuredIterations);
    const subgroupAvailable = device.features.has('subgroups') && device.wgslLanguageFeatures.has('subgroup_id');
    const strategies = subgroupAvailable
        ? ['portable', 'subgroups']
        : ['portable'];
    const expectedChecksum = getExpectedChecksum(roundCount);
    const timestampQueries = device.features.has('timestamp-query');
    const paths = strategies.map(strategy => makeBenchmarkPath(device, id, strategy, workgroupCount, roundCount, dispatchCount));
...
export function summarizeGPUWorkgroupScanBenchmarkSamples(samples) {
...
function makeBenchmarkPath(device, benchmarkId, strategy, workgroupCount, roundCount, dispatchCount) {
    ...
    const output = graph.importBuffer({ id: 'output', byteLength: outputBuffer.byteLength, usage: outputBuffer.usage }, outputBuffer);
    graph.addComputePass({
        ...
        compile: () => {
            const computation = new Computation(device, {
                id: `${benchmarkId}-${strategy}-local-scan`,
                source: strategy === 'subgroups' ? getSubgroupShader(roundCount) : getPortableShader(roundCount),
                shaderLayout: {
...
function encodeAndSubmit(device, path, id) {
...
async function validatePath(path, expectedChecksum, workgroupCount, benchmarkId) {
    ...
    if (values.some(value => value !== expectedChecksum)) {
        throw new Error(`${benchmarkId} ${path.strategy} path does not match the workgroup checksum oracle`);
    }
}
function getExpectedChecksum(roundCount) {
...
function validateProps(device, workgroupCount, roundCount, dispatchCount, warmupIterations, measuredIterations) {
    ...
    if (device.limits.maxComputeInvocationsPerWorkgroup < WORKGROUP_SIZE) {
        throw new Error(`GPU workgroup scan benchmark requires ${WORKGROUP_SIZE} workgroup invocations`);
    }
...
function getPortableShader(roundCount) {
    return /* wgsl */ `
...
function getSubgroupShader(roundCount) {
    return /* wgsl */ `
enable subgroups;
...
//# sourceMappingURL=gpu-workgroup-scan-benchmark.js.map