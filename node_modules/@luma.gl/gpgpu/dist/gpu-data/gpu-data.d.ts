...
import { type GPUDataFormat, type GPUDataFormatDeclaration, type GPUDataStructFields, type GPUDataStructFormat, type GPUDataStructLayout } from "./gpu-data-format.js";
...
type GPUDataFormatT<Format extends GPUDataFormatDeclaration, Layout extends GPUDataLayout> = Layout extends GPUDataStructLayout ? GPUDataStructFormat<Extract<Format, GPUDataStructFields>, Layout> : Extract<Format, GPUVectorFormat>;
 ...
 */
export type GPUDataFromBufferProps<Format extends GPUDataFormatDeclaration = GPUVectorFormat, Layout extends GPUDataStructLayout = 'wgsl-storage'> = GPUDataFromBufferBaseProps & ({
    ...
    layout?: never;
} | ([Extract<Format, GPUDataStructFields>] extends [never] ? never : {
...
type GPUDataStructFromBufferProps<Fields extends GPUDataStructFields, Layout extends GPUDataStructLayout> = GPUDataFromBufferBaseProps & {
    format: Fields;
    layout?: Layout;
};
/**
 ...
 */
export type GPUData<Format extends GPUDataFormatDeclaration = GPUVectorFormat, Layout extends GPUDataLayout = ScalarGPUDataLayout> = Omit<GPUDataBase<GPUDataFormatT<Format, Layout>>, 'getChild' | 'getChildAt'> & GPUDataChildMethods<Format, Layout>;
/** Selects broad scalar child methods or field-aware struct child methods. */
type GPUDataChildMethods<Format extends GPUDataFormatDeclaration, Layout extends GPUDataLayout> = Layout extends GPUDataStructLayout ? Extract<Format, GPUDataStructFields> extends infer Fields extends GPUDataStructFields ? {
...
type GPUDataConstructor = {
    ...
    new <Format extends GPUVectorFormat = GPUVectorFormat>(props: GPUDataScalarFromBufferProps<Format>): GPUData<Format>;
    /** Constructs struct data using the default `wgsl-storage` layout. */
    new <Fields extends GPUDataStructFields>(props: GPUDataStructFromBufferProps<Fields, 'wgsl-storage'>): GPUData<Fields, 'wgsl-storage'>;
    /** Constructs struct data using an explicitly selected layout. */
    new <Fields extends GPUDataStructFields, Layout extends GPUDataStructLayout>(props: GPUDataStructFromBufferProps<Fields, Layout> & {
        layout: Layout;
    }): GPUData<Fields, Layout>;
    /** Broad runtime signature used when callers do not retain literal format information. */
    new (props: GPUDataScalarFromBufferProps<GPUVectorFormat> | GPUDataStructFromBufferProps<GPUDataStructFields, GPUDataStructLayout>): GPUData;
};
/**
 ...
 */
export declare const GPUData: GPUDataConstructor;
/**
...
] extends [never] ? never : Extract<Format, GPUDataStructFields> extends infer Fields extends GPUDataStructFields ? string extends Name ? GPUDataChildAt<Fields> : Name extends keyof Fields ? GPUDataView<Fields[Name]> : never : never;
/**
...
export type GPUDataChildAt<Format extends GPUDataFormatDeclaration> = [
    Extract<Format, GPUDataStructFields>
] extends [never] ? never : Extract<Format, GPUDataStructFields> extends infer Fields extends GPUDataStructFields ? {
    [Name in keyof Fields]: GPUDataView<Fields[Name]>;
}[keyof Fields] : never;
export {};
//# sourceMappingURL=gpu-data.d.ts.map