...
export const LU_GRAPH_BENCHMARK_BOUNDS = [-2, -2, 2, 2];
/**
 ...
 */
export function makeGPUGraphBenchmarkDataset(options) {
    ...
    const weights = Float32Array.from(sources, (source, edgeIndex) => Math.fround(0.25 +
...
export function prepareGPUGraphBenchmark(options) {
...
export function evaluateGPUGraphBenchmarkShortestPath(adjacency) {
    const vertexCount = adjacency.forwardOffsets.length - 1;
    ...
    for (let iteration = 0; iteration < maximumIterations; iteration++) {
        ...
        for (let source = 0; source < vertexCount; source++) {
            ...
            for (let slot = adjacency.forwardOffsets[source]; slot < adjacency.forwardOffsets[source + 1]; slot++) {
                ...
                else if (distance === weightedDistances[target] &&
...
function evaluateGPUGraphBenchmarkLabelPropagation(adjacency, iterations) {
    ...
    for (let iteration = 0; iteration < iterations; iteration++) {
        ...
        for (let vertex = 0; vertex < vertexCount; vertex++) {
            const votes = new Map([[communities[vertex], 1]]);
        ...
        for (let vertex = 0; vertex < dataset.vertexCount; vertex++) {
            ...
            if (cells) {
                ...
                for (let cell = 0; cell < cells.length; cell++) {
                    ...
                    const useMonopole = !isNear &&
                        options.theta > 0 &&
                        diameterSquared < options.theta * options.theta * distanceSquared;
                    if (useMonopole) {
                        const scale = (repulsion * occupants.length) /
                        ...
                        for (const otherVertex of occupants) {
                            ...
                            const scale = repulsion /
                                Math.max(differenceToVertexX * differenceToVertexX +
                                    differenceToVertexY * differenceToVertexY, MINIMUM_REPULSION_DISTANCE_SQUARED);
                ...
                for (let otherVertex = 0; otherVertex < dataset.vertexCount; otherVertex++) {
                    ...
                    const scale = repulsion /
                        Math.max(differenceX * differenceX + differenceY * differenceY, MINIMUM_REPULSION_DISTANCE_SQUARED);
                    forceX += differenceX * scale;
                    forceY += differenceY * scale;
                }
            }
            ...
            let velocityX = ((intermediate ? intermediate[vertex * 2] : velocities[vertex * 2]) + forceX * timeStep) *
                damping;
            let velocityY = ((intermediate ? intermediate[vertex * 2 + 1] : velocities[vertex * 2 + 1]) +
                forceY * timeStep) *
                damping;
...
//# sourceMappingURL=gpu-graph-benchmark-data.js.map