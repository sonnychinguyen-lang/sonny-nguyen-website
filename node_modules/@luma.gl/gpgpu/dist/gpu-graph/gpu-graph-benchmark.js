...
import { GPUCommandGraph } from "../gpu-core/gpu-command-graph.js";
import { GPUGridIndex } from "../gpu-core/gpu-grid-index.js";
import { GPUGraph } from "./gpu-graph.js";
import { LU_GRAPH_BENCHMARK_BOUNDS, LU_GRAPH_BENCHMARK_FORCE_PROPS, LU_GRAPH_BENCHMARK_LABEL_ITERATIONS, getGPUGraphBenchmarkTime, prepareGPUGraphBenchmark, summarizeGPUGraphBenchmarkSamples } from "./gpu-graph-benchmark-data.js";
import { GPUGraphBreadthFirstSearch } from "./gpu-graph-breadth-first-search.js";
import { GPUGraphConnectedComponents } from "./gpu-graph-connected-components.js";
import { GPUGraphForceLayout } from "./gpu-graph-force-layout.js";
import { GPUGraphLabelPropagation } from "./gpu-graph-label-propagation.js";
import { GPUGraphLocalClusteringCoefficient } from "./gpu-graph-local-clustering-coefficient.js";
import { GPUGraphPageRank } from "./gpu-graph-page-rank.js";
import { GPUGraphSingleSourceShortestPath } from "./gpu-graph-single-source-shortest-path.js";
import { GPUGraphSpatialForceLayout } from "./gpu-graph-spatial-force-layout.js";
import { GPUGraphTopology } from "./gpu-graph-topology.js";
 ...
 */
export async function runGPUGraphBenchmark(device, options) {
    ...
    try {
        ...
        for (const path of compiledPaths) {
            ...
            const gpuSamples = executions.flatMap(execution => execution.gpuTimeMilliseconds === undefined ? [] : [execution.gpuTimeMilliseconds]);
            paths.push({
                algorithm: path.algorithm,
                ...(finalValidation.converged === undefined
                    ? {}
                    : {
                        iterations: path.algorithm === 'connected-components'
                ...
                ...(gpuSamples.length > 0
                    ? { gpuTimeMilliseconds: summarizeGPUGraphBenchmarkSamples(gpuSamples) }
                    : {}),
...
class GPUGraphBenchmarkResources {
    device;
    dataset;
    buffers = [];
    vectors = [];
...
async function executeBenchmarkPath(device, compiled, identifier, requestTimestamps = false) {
    const querySet = requestTimestamps
...
async function readBenchmarkPath(resources, context, algorithm) {
    ...
    const tolerance = algorithm === 'page-rank'
        ? PAGE_RANK_TOLERANCE
...
//# sourceMappingURL=gpu-graph-benchmark.js.map