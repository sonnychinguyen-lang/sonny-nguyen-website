...
import { getBoundedDispatchLayout, getBoundedInvocationIndexSource } from "../gpu-core/gpu-dispatch-utils.js";
import { createTransientView, getViewBinding, getViewElementOffset } from "../gpu-core/graph-data-view-utils.js";
import { GPUGraphModularity } from "./gpu-graph-modularity.js";
...
export function addGPUGraphModularityOptimizationToGraphWithDispatchLimit(optimization, commandGraph, maxComputeWorkgroupsPerDimension) {
...
function addControlInitializationPass(commandGraph, state) {
    const bindings = {
        ...
        ...(state.reverseOverflow
            ...
            : {}),
        ...(state.converged
            ? { converged: { view: state.converged, usage: 'storage-write' } }
            : {})
    };
...
function addCommunityInitializationPass(commandGraph, state) {
    const bindings = {
        ...
        ...(state.initialCommunities
            ? { initialCommunities: { view: state.initialCommunities, usage: 'storage-read' } }
            : {}),
        control: { view: state.control, usage: 'storage-read-write', atomic: true }
    };
...
function addFailureFinalizationPass(commandGraph, state) {
    const bindings = {
        ...
        control: { view: state.control, usage: 'storage-read-write', atomic: true },
        ...(state.converged
            ? { converged: { view: state.converged, usage: 'storage-read-write' } }
            : {})
    };
...
export function getGPUGraphModularityOptimizationDispatchLayout(elementCount, maxComputeWorkgroupsPerDimension) {
...
//# sourceMappingURL=gpu-graph-modularity-optimization-internals.js.map