...
import { addGPUScanToGraphWithDispatchLimit, GPUScan } from "../gpu-core/gpu-scan.js";
...
export function addGPUGraphTopologyToGraphWithDispatchLimit(topology, commandGraph, maxComputeWorkgroupsPerDimension) {
    const sources = {
    ...
    for (const [chunkIndex, sourceVertices] of props.sources.sourceVertices.data.entries()) {
        if (sourceVertices.length > 0 && props.adjacency.neighbors.length > 0) {
            ...
            const slots = edgeWeights
                ? createTransientView(commandGraph, `${props.id}-slots-${chunkIndex}`, 'uint32', sourceVertices.length * (props.directed ? 1 : 2))
                : undefined;
...
export function getGPUGraphTopologyDispatchLayout(elementCount, maxComputeWorkgroupsPerDimension) {
...
//# sourceMappingURL=gpu-graph-topology-internals.js.map