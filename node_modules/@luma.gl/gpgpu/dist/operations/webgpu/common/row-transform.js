...
import { getWebGPUDispatchLayout, getWebGPUDispatchRowIndex } from "./dispatch.js";
import { getLiteralValue, getWGSLType, getZeroValue } from "./helper.js";
...
export function runRowComputation({ module, elementWise = false, expression, inputs, output, operationType = output.type, outputBuffer }) {
    if (!module.source) {
...
${Array.from({ length: input.size }, (_, elementIndex) => castToType
        ? `  value[${elementIndex}] = ${castToType}(${name}[rowOffset + ${elementIndex}u]);`
        : `  value[${elementIndex}] = ${name}[rowOffset + ${elementIndex}u];`).join('\n')}
  return value;
}`;
}
