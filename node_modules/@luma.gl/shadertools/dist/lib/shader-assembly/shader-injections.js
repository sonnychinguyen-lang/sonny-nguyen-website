...
export function injectShader(source, stage, inject, injectStandardStubs = false, language = 'glsl', entryPoints = {}) {
        ...
        switch (key) {
                ...
                if (language === 'wgsl' || isVertex) {
                    source =
                        language === 'wgsl'
                            ...
                            : source.replace(REGEX_START_OF_MAIN, (match) => match + fragmentString);
                }
                break;
                ...
                if (language === 'wgsl' || isVertex) {
                    source =
                        language === 'wgsl'
                            ...
                            : source.replace(REGEX_END_OF_MAIN, (match) => fragmentString + match);
                }
                break;
            // declarations are injected before the main function
            case 'fs:#decl':
            ...
            case 'fs:#main-start':
                if (language === 'wgsl' || !isVertex) {
                    source =
                        language === 'wgsl'
                            ? injectWGSLStageMain(source, 'fragment', fragmentString, 'start', entryPoints.fragment)
                            : source.replace(REGEX_START_OF_MAIN, (match) => match + fragmentString);
                }
                break;
            // inject code at the end of main function
