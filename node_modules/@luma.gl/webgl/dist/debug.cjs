...
function registerWebGLDeveloperTools(hooks) {
  webglDeveloperToolsHooks = hooks;
}
function registerSpectorJS(hooks) {
  spectorHooks = hooks;
}
...
function onGLError(props, err, functionName, args) {
  ...
  import_core2.log.error("%cWebGL", "color: white; background: red; padding: 2px 6px; border-radius: 3px;", message)();
  debugger;
...
var DEFAULT_SPECTOR_PROPS = {
  debugSpectorJS: import_core3.log.get("debug-spectorjs"),
...
async function loadSpectorJS(props) {
  if (!globalThis.SPECTOR) {
    try {
      ...
      import_core3.log.warn(String(error));
    }
  }
}
function initializeSpectorJS(props) {
  ...
  if (!spector && globalThis.SPECTOR && !((_a = globalThis.luma) == null ? void 0 : _a.spector)) {
    import_core3.log.probe(LOG_LEVEL, "SPECTOR found and initialized. Start with `luma.spector.displayUI()`")();
  ...
  if (!initialized) {
    initialized = true;
    spector.spyCanvases();
    spector == null ? void 0 : spector.onCaptureStarted.add((capture) => import_core3.log.info("Spector capture started:", capture)());
    spector == null ? void 0 : spector.onCapture.add((capture) => {
      import_core3.log.info("Spector capture complete:", capture)();
  ...
  if (props.gl) {
    const gl = props.gl;
    const contextData = getWebGLContextData2(gl);
    ...
    new Promise((resolve) => setTimeout(resolve, 2e3)).then((_) => {
      import_core3.log.info("Spector capture stopped after 2 seconds")();
