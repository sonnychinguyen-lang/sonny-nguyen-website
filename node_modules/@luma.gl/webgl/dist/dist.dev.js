...
var __exports__ = (() => {
  ...
  // src/context/debug/debug-hooks.ts
  ...
  var import_core4, WebGLDeviceLimits;
    ...
    "src/adapter/device-helpers/webgl-device-limits.ts"() {
      ...
      WebGLDeviceLimits = class extends import_core4.DeviceLimits {
        ...
        get maxColorAttachments() {
          return this.getParameter(36063 /* MAX_COLOR_ATTACHMENTS */);
        }
  ...
  var import_core5, WEBGLFramebuffer;
      ...
      WEBGLShader = class extends import_core9.Shader {
        ...
        constructor(device, props) {
          ...
          switch (this.props.stage) {
            case "vertex":
              this.handle = suppliedHandle || this.device.gl.createShader(35633 /* VERTEX_SHADER */);
              break;
            case "fragment":
              this.handle = suppliedHandle || this.device.gl.createShader(35632 /* FRAGMENT_SHADER */);
              break;
            default:
...
${source}`;
        ...
        _getCompilationErrorMessage(source) {
          ...
          const sourceLine = firstMessage.lineNum ? source.split(/\r?\n/)[firstMessage.lineNum - 1]?.trim() : void 0;
...
Source: ${sourceLine}` : "";
  ...
  function readUniformBlocks(gl, program, options) {
    ...
    for (const [blockIndex, uniformBlock] of knownBlocksByIndex) {
      ...
      try {
        ...
        import_core14.log.once(
          0,
          `WebGL uniform block reflection failed for "${uniformBlock.name}"; using supplied std140 metadata. ${message2}`
        )();
      }
    }
  ...
  function validateReflectedUniformBlock(reflectedUniformBlock, knownUniformBlock) {
    for (const reflectedUniform of reflectedUniformBlock.uniforms) {
      const knownUniform = knownUniformBlock.uniforms.find(
        (uniform) => reflectedUniform.name === uniform.name || reflectedUniform.name.endsWith(`.${uniform.name}`)
      );
      if (!knownUniform) {
        throw new Error(
  ...
  function getActiveUniformBlockIndex(gl, program, blockName) {
    ...
    for (const candidateBlockName of blockNames) {
      ...
      if (blockIndex !== 4294967295 /* INVALID_INDEX */) {
  ...
  function readReflectedUniformBlock(gl, program, blockIndex, knownBlockName) {
    ...
    const location = getFiniteInteger(
      getBlockParameter(35391 /* UNIFORM_BLOCK_BINDING */, "UNIFORM_BLOCK_BINDING"),
      blockName,
      "UNIFORM_BLOCK_BINDING",
      0
    );
    const byteLength = getFiniteInteger(
      getBlockParameter(35392 /* UNIFORM_BLOCK_DATA_SIZE */, "UNIFORM_BLOCK_DATA_SIZE"),
      blockName,
      "UNIFORM_BLOCK_DATA_SIZE",
      0
    );
    const uniformCount = getFiniteInteger(
      getBlockParameter(35394 /* UNIFORM_BLOCK_ACTIVE_UNIFORMS */, "UNIFORM_BLOCK_ACTIVE_UNIFORMS"),
      blockName,
  ...
  function isUsableUniformBlockBinding(binding) {
    return binding.type === "uniform" && Number.isInteger(binding.minBindingSize) && binding.minBindingSize >= 0 && Array.isArray(binding.uniforms) && binding.uniforms.every(
      (uniform) => typeof uniform.name === "string" && typeof uniform.format === "string" && Number.isInteger(uniform.arrayLength) && uniform.arrayLength > 0 && Number.isInteger(uniform.byteOffset) && uniform.byteOffset >= 0 && Number.isInteger(uniform.byteStride) && uniform.byteStride >= 0
    );
  }
  function parseUniformName(name) {
    ...
    const uniformName = (0, import_core14.assertDefined)(matches?.[1], `Failed to parse GLSL uniform name ${name}`);
    return {
      name: uniformName,
  ...
  var import_core16, LOG_PROGRAM_PERF_PRIORITY, WEBGLSharedRenderPipeline;
    ...
    "src/adapter/resources/webgl-render-pass.ts"() {
      ...
      WEBGLRenderPass = class extends import_core18.RenderPass {
        ...
        setBindings(bindings, _options) {
          ...
          this.bindings = (0, import_core18.flattenBindingsByGroup)(
            (0, import_core18.normalizeBindingsByGroup)(this.pipeline.shaderLayout, bindings)
          );
      ...
      WEBGLVertexArray = class _WEBGLVertexArray extends import_core20.VertexArray {
        ...
        constructor(device, props) {
          ...
          for (const attributeInfo of Object.values(
            (0, import_core20.getAttributeInfosFromLayouts)(props.shaderLayout, props.bufferLayout)
          )) {
